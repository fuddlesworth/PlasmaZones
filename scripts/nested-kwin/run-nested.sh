...
# only if a guard refuses a session you know has been stopped. Services the
# nested bus activated are stopped when the compositor exits, and again at
# the next launch for a session that died before it could (see
# sweep-activated.sh below).
#
...
# Services the session bus activated outlive it. dbus-run-session stops the
# bus, but some activated services (xdg-desktop-portal-kde, ksecretd and
# kactivitymanagerd, observed) do not exit when their bus goes away. They keep
# writing into the nested XDG homes, so the next launch's `rm -rf` of $HOME_N
# fails with "Directory not empty" and the launcher refuses to start.
#
# The sweep is identity-gated on DBUS_STARTER_ADDRESS, which dbus-daemon puts
# in the environment of every service IT activated and nothing else carries:
# not the harness, not the shell or daemon it starts, and never a terminal the
# user sourced env.sh into (a shell's `export` does not rewrite its
# /proc/<pid>/environ). The address embeds the bus's unique guid, so a match is
# this session's bus and no other. The session runs it when the compositor
# exits (see the sh -c below); the launch-time call here is the backstop for a
# session that never got to, such as one killed with SIGKILL.
mkdir -p "$NEST"
cat > "$NEST/sweep-activated.sh" <<'SWEEP'
#!/usr/bin/env bash
# Generated by run-nested.sh. Stops the services one nested bus activated.
# Usage: sweep-activated.sh <full DBUS_SESSION_BUS_ADDRESS of that bus>
addr="${1:-}"
[ -n "$addr" ] || exit 0
targets=()
for d in /proc/[0-9]*; do
    [ -O "$d" ] || continue
    # Redirect stderr before the input redirect: a zombie's environ fails to
    # open with ESRCH, and bash reports a failed redirect itself, left to right.
    tr '\0' '\n' 2>/dev/null < "$d/environ" | grep -Fqx "DBUS_STARTER_ADDRESS=$addr" || continue
    targets+=("${d#/proc/}")
done
[ "${#targets[@]}" -gt 0 ] || exit 0
...
SWEEP
chmod +x "$NEST/sweep-activated.sh"
if [ -f "$NEST/env.sh" ]; then
    OLDADDR=$(sed -n "s/^export DBUS_SESSION_BUS_ADDRESS='\(.*\)'\$/\1/p" "$NEST/env.sh" 2>/dev/null | head -n1)
    "$NEST/sweep-activated.sh" "$OLDADDR"
fi
...
exec dbus-run-session -- sh -c "
  ...
  # word splitting.
  #
  # kwin_wayland runs as a child rather than an exec so this shell is still
  # here when it exits, with the bus still up (dbus-run-session waits for this
  # shell), to stop what the bus activated. The trap keeps a Ctrl+C or a hangup
  # from ending this shell before the sweep: a trapped signal waits for the
  # foreground command, and kwin itself gets the default disposition back.
  trap : INT TERM HUP
  status=0
  kwin_wayland $BACKEND_FLAGS$EXTRA_FLAGS --socket '$PZ_NESTED_SOCKET' --no-lockscreen --no-global-shortcuts --no-kactivities || status=\$?
  '$NEST/sweep-activated.sh' \"\$DBUS_SESSION_BUS_ADDRESS\" || true
  exit \$status
"
