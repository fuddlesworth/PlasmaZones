...
class RuleStoreWatcher::ReloadStrategy : public PhosphorFsLoader::IScanStrategy
{
public:
    explicit ReloadStrategy(RuleStore& store)
        : m_store(&store)
    {
    }
...
RuleStoreWatcher::RuleStoreWatcher(RuleStore& store, QObject* parent)
    : QObject(parent)
    , m_store(&store)
...
RuleStoreWatcher::~RuleStoreWatcher() = default;

void RuleStoreWatcher::start()
{
    if (m_started) {
        return;
    }
    // RuleStore asserts an empty path in debug and critically logs it in
    ...
    const QString filePath = m_store->filePath();
    if (filePath.isEmpty()) {
        qCWarning(lcRule) << "RuleStoreWatcher: store has an empty file path — watcher is inert";
        return;
    }
    m_started = true;
    // Watch the directory that holds rules.json. registerDirectory with
