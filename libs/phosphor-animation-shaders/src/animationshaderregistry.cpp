...
///
/// Mirrors `PhosphorShaders::ShaderRegistry::ShaderScanStrategy` — same
                                             ...
                                             PhosphorFsLoader::RegistrationOrder order)
{
    ...
    // run upstream). Sister `PhosphorShaders::ShaderRegistry` uses the
...
QList<AnimationShaderEffect> AnimationShaderRegistry::availableEffects() const
{
    ...
    // in `PhosphorShaders::ShaderRegistry::availableShaders` for consistency
...
QStringList AnimationShaderRegistry::performScan(const QStringList& directoriesInScanOrder)
{
    ...
    // `JsScanStrategy`, and `PhosphorShaders::ShaderRegistry`. Caller
