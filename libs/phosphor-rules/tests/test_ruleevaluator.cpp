...
private Q_SLOTS:
    ...
    {
        RuleSet set;
        Rule r = makeRule(QStringLiteral("disabled"), 100, MatchExpression{}, {floatAction()});
        r.enabled = false;
        set.addRule(r);
        RuleEvaluator eval(set);
        QVERIFY(eval.resolve(konsoleQuery()).isEmpty());
    ...
    void testModeConditionResolvesPerMode()
    {
        ...
        QVERIFY(!eval.resolve(konsoleQuery()).hasSlot(QString(ActionSlot::InnerGap)));
    }
    ...
    void testHasMatchTargetingFields()
    {
        ...
        RuleSet disabled;
        Rule r = makeRule(QStringLiteral("transient rule"), 100,
