...
| `PhosphorRegistry::ILauncherProviderFactory` | Factory for a launcher query provider (apps, calculator, etc.). Adds `createProvider(QObject*) -> ILauncherProvider*`. Consumed by `libs/phosphor-shell-launcher`, over the registry `src/shell/LauncherController` populates. |
