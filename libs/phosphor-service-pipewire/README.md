...
| `PwNodeModel`         | Abstract `QAbstractListModel` filtering a connection's nodes by `mediaClasses`. 10 pinned roles: `node`, `id`, `name`, `nick`, `description`, `mediaClass`, `channelCount`, `volumes`, `muted`, plus `Qt::DisplayRole` (`display`) that falls back through nick → description → name. |
...
| `PipeWireHost`        | QML singleton (`Phosphor.Service.PipeWire 1.0`) that owns the process-wide `PipeWireConnection` and forwards every observable signal. Auto-connects on construction; exposes `reconnect()` for explicit retry. |
