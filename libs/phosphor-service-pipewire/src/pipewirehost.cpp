    ...
    , d(std::make_unique<Private>())
{
    ...
    // Forward every observable signal so QML can bind through
    // PipeWireHost without reaching for `.connection.foo` everywhere.
    // The forwarding is symmetric (no transformation) — the host is a
    // facade, not a controller.
