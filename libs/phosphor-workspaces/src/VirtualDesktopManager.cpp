...
void VirtualDesktopManager::applyDesktopListReply(const QDBusMessage& reply)
{
    ...
    // kept its previous value, and the "ids and names are index-aligned"
    // contract this class publishes would be broken with no way back until
    // some unrelated signal forced another refresh. Every id-keyed operation
    // fails silently in that state: the pager empties, and both
    // setCurrentDesktopById and setCurrentDesktop drop every switch because
    // they validate against the empty id list.
    //
    ...
    // Which POSITION went, while the previous list is still here to compare
    ...
    // one-out-none-in case: anything else and consumers fall back to the count
    // handler's out-of-range sweep.
    //
    ...
    // handler's "prune everything past newCount" sweep correctly finds nothing
    // left to do.
    int removedPosition = 0;
    if (m_desktopIds.size() == previousIds.size() - 1) {
        const QSet<QString> survivors(m_desktopIds.cbegin(), m_desktopIds.cend());
        for (int i = 0; i < previousIds.size(); ++i) {
            if (!survivors.contains(previousIds.at(i))) {
        ...
        if (removedPosition > 0) {
            const QSet<QString> previous(previousIds.cbegin(), previousIds.cend());
            for (const QString& id : std::as_const(m_desktopIds)) {
    ...
    if (removedPosition > 0) {
        ...
        // active-layout map built from it, and only setDesktopCount's clamp
        ...
        const int newCount = m_desktopIds.size();
        for (auto it = m_screenDesktops.begin(); it != m_screenDesktops.end(); ++it) {
            if (it.value() > newCount) {
                it.value() = newCount;
            }
        }
...
void VirtualDesktopManager::onNumberOfDesktopsChanged(uint count)
{
    ...
    // Boundary validation: KWin never emits 0, but this is a session-bus
    // signal any peer can send, and committing 0 would publish an invalid
    // 1-based currentDesktopChanged(0) below until a refresh corrected it.
    // The upper bound keeps the narrowing cast below well-defined.
    if (count < 1 || count > static_cast<uint>(std::numeric_limits<int>::max())) {
        return;
    }
