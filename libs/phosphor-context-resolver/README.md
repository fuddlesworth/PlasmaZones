...
> disable/lock cascade. Replaces 22+ hand-stitched five-call chains
> across the daemon, three D-Bus adaptors, and the KWin effect with one
> resolver call.
...
- **The adapter contracts:** `IWorkspaceState`, `IModeProvider`,
  `IContextGateSource` — three small interfaces that consumers wire
...
- **The concrete resolver:** `ContextResolver` borrows three adapter
  pointers, composes the cascade, owns no state of its own.
...
| `PhosphorContext::IModeProvider`         | Adapter: "what mode is this screen?" |
| `PhosphorContext::IContextGateSource`    | Adapter: "is this `(mode, screen, desktop, activity)` disabled / locked?" |
...
- **No mutation surface.** Toggling a disable list, setting a lock,
  changing the active mode — all of that still goes through the
