                                                                           ...
                                                                           const QString& activity) const
{
    ...
    // canonical id, fall back to a shape-based scan that also picks up
    // user-authored exact-context rules whose UUIDs were generated by
    // the settings UI (windowruletemplates.cpp's `QUuid::createUuid()`
    // path). Without the fallback, `hasExplicitAssignment` returns false
    // on user-authored rules, `upsertAssignmentRule` adds a duplicate at
    // the same priority, and `clearAssignment` fails to remove the
    // user's rule — silent data corruption.
    ...
    for (const PWR::WindowRule& rule : m_ruleStore->ruleSet().rules()) {
        ...
        // Remember the first shape-matching rule we see — we'll only
        // return it if no deterministic-id rule exists in the set.
        if (shapeMatch == nullptr && hasEngineModeAction(rule)
