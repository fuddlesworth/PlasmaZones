...
private:
    /// columnOfWindow with the -1 miss turned into a failed assertion rather
    /// than an out-of-range .at() or a vacuous -1 == -1 compare.
...
void TestScrollEngineBehaviour::sameAppGroupingYieldsToRulesAndUnstableIds()
{
    ...
    QCOMPARE(columnOfOrFail(state, QStringLiteral("kate|2")), columnOfOrFail(state, QStringLiteral("kate|1")));
    QVERIFY(columnOfOrFail(state, QStringLiteral("kate|2")) != workCol);
...
void TestScrollEngineBehaviour::openTabbedFalseOptsOutOfAppGrouping()
{
...
void TestScrollEngineBehaviour::groupedJoinUnderFocusNewOffKeepsTheHostsShownTab()
{
...
void TestScrollEngineBehaviour::groupingSkipsMigrationsUnfloatsAndOutrankedArms()
{
...
void TestScrollEngineBehaviour::groupingSkipsDraggedAndMinimizedHosts()
{
    ...
    engine->windowOpened(QStringLiteral("firefox|2"), kS1, 0, 0);
    QCOMPARE(state->strip().columnCount(), 3);
    QCOMPARE(state->strip().columns().at(columnOfOrFail(state, QStringLiteral("firefox|1"))).display,
