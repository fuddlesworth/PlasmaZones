...
bool ScrollStrip::setActiveWindowHeight(const WindowHeight& height)
{
    ...
    // Above the bail, unlike adjust, minimize, expand and the preset cycle.
    // Those COMPUTE a height and then decline when it would change nothing, so
    // their bail is a refusal and clearing there would let a held-down key at
    // its limit wipe the memory. Here the CALLER named the height and got it,
    // even when the tile already held that value, so the maximize it may have
    // been sitting in is over. Reachable because a slot-bearing tile holds
    // exactly Fixed(the budget at maximize time), which a rule or a D-Bus
    // caller can name. Tile's note carries the whole division.
