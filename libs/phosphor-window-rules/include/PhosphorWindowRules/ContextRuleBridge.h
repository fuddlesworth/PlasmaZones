...
using PhosphorWindowRules::Detail::encodeSegment;
 ...
 * carry any non-empty token without coupling PhosphorWindowRules to
...
inline std::optional<QString> disableRuleMode(const WindowRule& rule)
{
    ...
    // PhosphorWindowRules lib does not need to depend on PhosphorZones.
    return mode;
}
