...
/**
 * @brief An ordered collection of WindowRules — the persistent rule store.
 *
 * Serializes to `windowrules.json` with `"_version": 4`. The set carries a
 ...
 */
class PHOSPHORWINDOWRULES_EXPORT WindowRuleSet
{
public:
    /// The schema version this library reads and writes.
    static constexpr int SchemaVersion = 4;

    WindowRuleSet() = default;

    // ── Accessors ──
    const QList<WindowRule>& rules() const
    {
        return m_rules;
    }
    int count() const
    ...
    std::optional<WindowRule> ruleById(const QUuid& id) const;
    ...
    static std::optional<WindowRuleSet> fromJson(const QJsonObject& obj);
    ...
    static std::optional<WindowRuleSet> loadFromFile(const QString& path);
