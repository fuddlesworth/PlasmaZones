...
/**
 * @brief Opt-in cross-process auto-reload for a WindowRuleStore.
 *
 * Watches the store's backing @c windowrules.json and calls
 * @ref WindowRuleStore::load() when it changes on disk, so a separate-process
 ...
 *
 * Self-writes need no special handling: @ref WindowRuleStore::load() is
 ...
 * the behaviour, so the base @ref WindowRuleStore (and the daemon) carry no
 * watcher overhead.
 *
 ...
 */
class PHOSPHORWINDOWRULES_EXPORT WindowRuleStoreWatcher : public QObject
{
    Q_OBJECT
...
public:
     ...
     */
    explicit WindowRuleStoreWatcher(WindowRuleStore& store, QObject* parent = nullptr);
    ~WindowRuleStoreWatcher() override;

    WindowRuleStoreWatcher(const WindowRuleStoreWatcher&) = delete;
    WindowRuleStoreWatcher& operator=(const WindowRuleStoreWatcher&) = delete;
     ...
     * because @ref WindowRuleStore::load() is idempotent, that is a no-op when
