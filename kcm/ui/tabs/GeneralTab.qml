                    ...
                    Kirigami.FormLayout {
                        ...
                        ComboBox {
                            ...
                            readonly property var presets: [
                                ...
                                // --- Standard ---
                                { name: i18n("--- Standard ---"), curve: "__section__" },
                                { name: i18n("Linear"), curve: "0.00,0.00,1.00,1.00" },
                                // --- Sine ---
                                { name: i18n("--- Sine ---"), curve: "__section__" },
                                { name: i18n("Ease In (Sine)"), curve: "0.12,0.00,0.39,0.00" },
                                { name: i18n("Ease Out (Sine)"), curve: "0.61,1.00,0.88,1.00" },
                                { name: i18n("Ease In-Out (Sine)"), curve: "0.37,0.00,0.63,1.00" },
                                // --- Quad ---
                                { name: i18n("--- Quad ---"), curve: "__section__" },
                                { name: i18n("Ease In (Quad)"), curve: "0.11,0.00,0.50,0.00" },
                                { name: i18n("Ease Out (Quad)"), curve: "0.50,1.00,0.89,1.00" },
                                { name: i18n("Ease In-Out (Quad)"), curve: "0.45,0.00,0.55,1.00" },
                                // --- Cubic ---
                                { name: i18n("--- Cubic ---"), curve: "__section__" },
                                { name: i18n("Ease In (Cubic)"), curve: "0.32,0.00,0.67,0.00" },
                                { name: i18n("Ease Out (Cubic)"), curve: "0.33,1.00,0.68,1.00" },
                                { name: i18n("Ease In-Out (Cubic)"), curve: "0.65,0.00,0.35,1.00" },
                                // --- Quart ---
                                { name: i18n("--- Quart ---"), curve: "__section__" },
                                { name: i18n("Ease In (Quart)"), curve: "0.50,0.00,0.75,0.00" },
                                { name: i18n("Ease Out (Quart)"), curve: "0.25,1.00,0.50,1.00" },
                                { name: i18n("Ease In-Out (Quart)"), curve: "0.76,0.00,0.24,1.00" },
                                // --- Quint ---
                                { name: i18n("--- Quint ---"), curve: "__section__" },
                                { name: i18n("Ease In (Quint)"), curve: "0.64,0.00,0.78,0.00" },
                                { name: i18n("Ease Out (Quint)"), curve: "0.23,1.00,0.32,1.00" },
                                { name: i18n("Ease In-Out (Quint)"), curve: "0.83,0.00,0.17,1.00" },
                                // --- Expo ---
                                { name: i18n("--- Expo ---"), curve: "__section__" },
                                { name: i18n("Ease In (Expo)"), curve: "0.70,0.00,0.84,0.00" },
                                { name: i18n("Ease Out (Expo)"), curve: "0.16,1.00,0.30,1.00" },
                                { name: i18n("Ease In-Out (Expo)"), curve: "0.87,0.00,0.13,1.00" },
                                // --- Circ ---
                                { name: i18n("--- Circ ---"), curve: "__section__" },
                                { name: i18n("Ease In (Circ)"), curve: "0.55,0.00,1.00,0.45" },
                                { name: i18n("Ease Out (Circ)"), curve: "0.00,0.55,0.45,1.00" },
                                { name: i18n("Ease In-Out (Circ)"), curve: "0.85,0.00,0.15,1.00" },
                                // --- Back ---
                                { name: i18n("--- Back ---"), curve: "__section__" },
                                { name: i18n("Ease In (Back)"), curve: "0.36,0.00,0.66,-0.56" },
                                { name: i18n("Overshoot"), curve: "0.18,0.89,0.32,1.28" },
                                { name: i18n("Overshoot (In-Out)"), curve: "0.68,-0.55,0.27,1.55" },
                                // --- Elastic ---
                                { name: i18n("--- Elastic ---"), curve: "__section__" },
                                { name: i18n("Elastic In"), curve: "elastic-in:1.0,0.30" },
                                { name: i18n("Elastic Out"), curve: "elastic-out:1.0,0.30" },
                                { name: i18n("Elastic In-Out"), curve: "elastic-in-out:1.0,0.30" },
                                // --- Bounce ---
                                { name: i18n("--- Bounce ---"), curve: "__section__" },
                                { name: i18n("Bounce In"), curve: "bounce-in:1.0,3" },
                                { name: i18n("Bounce Out"), curve: "bounce-out:1.0,3" },
                                { name: i18n("Bounce In-Out"), curve: "bounce-in-out:1.0,3" }
                            ]
                            ...
                            function normalizeCurve(curve) {
                                if (!curve || curve === "" || curve === "__section__") return ""
                                ...
                                if (hasLetter) {
                                    ...
                                    if (params) {
                                        ...
                                        var amp = parts.length >= 1 ? parseFloat(parts[0]) : 1.0
                                        if (!isFinite(amp)) amp = 1.0
                                        if (isElastic) {
                                            var per = parts.length >= 2 ? parseFloat(parts[1]) : 0.3
                                            if (!isFinite(per)) per = 0.3
                                    ...
                                    if (isBounce) return name + ":1.00,3"
                                    if (isElastic) return name + ":1.00,0.30"
                                    return name
                                }
                                // Bezier: normalize to 2 decimal places
                                var bparts = curve.split(",")
                                if (bparts.length !== 4) return ""
                            ...
                            function findPresetIndex(curve) {
                                ...
                                for (var i = 1; i < presets.length; i++) {
                                    if (presets[i].curve === "__section__") continue
                        ...
                        RowLayout {
                            ...
                            Slider {
                                ...
                                from: amplitudeRow.isElastic ? 1.0 : 0.5
                                to: 3.0
                                stepSize: 0.1
                                ...
                                ToolTip.visible: hovered
                                ToolTip.text: amplitudeRow.isBounce
                                    ? i18n("Controls the bounce height — higher values exaggerate bounces, lower values flatten them (1.0 = standard)")
                                    : i18n("Controls the overshoot intensity of the elastic animation (1.0 = standard)")
                            }
