...
RowLayout {
        ...
        model: {
            ...
            if (root.viewMode === 0)
                items.push({
                    ...
                    "enabled": true,
                    "action": () => {
                        return root.kcm.createNewLayout();
                    },
                    "tooltip": ""
                });
            ...
            items.push({
                ...
                "action": () => {
                    if (root.currentLayout)
                        root.kcm.editLayout(root.currentLayout.id);
            ...
            if (root.viewMode === 0) {
                items.push({
                    "text": i18n("Duplicate"),
                    "icon": "edit-copy",
                    "enabled": root.currentLayout !== null && !(root.currentLayout && root.currentLayout.isAutotile === true),
                    "action": () => {
                        if (root.currentLayout)
                            root.kcm.duplicateLayout(root.currentLayout.id);
                ...
                items.push({
                    "text": i18n("Delete"),
                    "icon": "edit-delete",
                    "enabled": root.currentLayout !== null && !(root.currentLayout && root.currentLayout.isSystem) && !(root.currentLayout && root.currentLayout.isAutotile),
                    "action": () => {
                        if (root.currentLayout)
                            root.requestDeleteLayout(root.currentLayout);
    ...
    Button {
        ...
        ToolTip.text: root.viewMode === 1 ? i18n("Set this algorithm as the default autotile algorithm") : i18n("Set this layout as the default for screens without specific assignments")
    }
    ...
    Button {
        ...
        onClicked: {
            if (root.currentLayout)
                root.requestExportLayout(root.currentLayout.id);
