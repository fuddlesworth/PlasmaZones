...
// metadata trailSeconds (1.7) is that sum.
...
// One complete response to a button event: converge, hold, fade. `duration`
// is the converge plus the fade and `hold` sits between them, so the whole
// response lasts duration + hold and Hold always lengthens it. Deriving the
// hold from the duration instead clamped the top of the Hold slider to
// nothing at short durations. Returns coverage in 0..1 and is exactly 0 at
// and past duration + hold.
float response(vec2 px, vec2 origin, float since, float duration, float hold, float spread, float half_) {
    if (since < 0.0 || since >= duration + hold) {
        return 0.0;
    }
    ...
    // exactly at duration + hold.
    float tail = max(duration - converge, 1e-4);
...
vec4 pPointer(vec2 uv) {
    ...
    if (uPointerPress.w > 0.5 && sincePress < total
        && max(abs(px.x - uPointerPress.x), abs(px.y - uPointerPress.y)) <= spread) {
        float a = response(px, uPointerPress.xy, sincePress, duration, hold, spread, half_);
    ...
    if (echo > 0.0 && uPointerRelease.w > 0.5 && sinceRelease < total
        && max(abs(px.x - uPointerRelease.x), abs(px.y - uPointerRelease.y)) <= spread) {
        float a = response(px, uPointerRelease.xy, sinceRelease, duration, hold, spread * 0.6, half_ * 0.7);
