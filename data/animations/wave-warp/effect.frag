...
vec4 pzTransition(vec2 uv, float t) {
    ...
    float p = clamp(iTime * pz_frontSpeed, 0.0, 1.0);
    ...
    vec2 dir = vec2(cos(pz_waveAngle), sin(pz_waveAngle));
    vec2 v = dir;
    v /= abs(v.x) + abs(v.y);
    float d = v.x * 0.5 + v.y * 0.5;
    float m = 1.0 - smoothstep(-pz_waveSmoothness, 0.0, v.x * uv.x + v.y * uv.y - (d - 0.5 + p * (1.0 + pz_waveSmoothness)));
