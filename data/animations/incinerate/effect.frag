...
// `pz_*` param macros are generated from metadata.json by the harness.
...
vec4 getFireColor(float val) {
  return vec4(tritone(val, vec3(0.0), pz_uColor.rgb, vec3(1.0)), val);
}
...
vec4 pzTransition(vec2 uv, float t) {
  ...
  float SCORCH_WIDTH = 0.2 * pz_uScale;
  float BURN_WIDTH   = 0.03 * pz_uScale;
  float SMOKE_WIDTH  = 0.9 * pz_uScale;
  float FLAME_WIDTH  = 0.2 * pz_uScale;
  ...
  float circle = length((iTexCoord - vec2(pz_uStartPosX, pz_uStartPosY)) * (uSize.xy / max(uSize.x, uSize.y)));

  vec2 noiseUv = iTexCoord / pz_uScale * uSize / 1.5;
  ...
  float smokeNoise =
    simplex2DFractal(noiseUv * 0.01 + vec2(pz_uSeedX, pz_uSeedY) + (float(iFrame) / 60.0) * vec2(0.0, 0.3));

  float gradient =
    mix(circle, smokeNoise, 200.0 * pz_uTurbulence * pz_uScale / max(uSize.x, uSize.y));
  ...
  if (smokeRange.x < gradient && gradient < smokeRange.y) {
    ...
    float emberNoise = simplex2DFractal(
      noiseUv * 0.05 + vec2(pz_uSeedX, pz_uSeedY) - smokeNoise * vec2(0.0, 0.3 * smokeMask * 1.0));
  ...
  if (min(burnRange.x, flameRange.x) < gradient &&
    ...
    float flameNoise =
      simplex2DFractal(noiseUv * 0.02 + vec2(pz_uSeedX, pz_uSeedY) + smokeNoise * vec2(0.0, 1.0) +
