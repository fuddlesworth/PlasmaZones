...
//
// Tab Fade — the plainest of the tab packs. The class ships NO built-in
// default (a fresh config hard-cuts until the user picks a pack, the same
// opt-in stance as desktop and strip); this is simply the first pack most
...
// main(). old_content.glsl (the shared uOldWindow + oldColor module) is
// included per pack rather than auto-prologued.
...
vec4 pTransition(vec2 uv, float t) {
    ...
    //
    // Clamped IN THE SHADER, not trusted to the metadata bounds: the shader
    // library's range check has no runtime caller, so a hand-edited profile
    // or a D-Bus param write reaches this uniform unclamped. crossPoint = 1.0
    // would be log(1) = 0, a divide by zero, and a NaN column; 0.0 would pin
    // the blend at "arrived instantly" with nothing logged. The clamp makes
