...
//
// Portal transition — ported from Burn-My-Windows' portal.frag
// (https://github.com/Schneegans/Burn-My-Windows). Star-Trek-style
// swirling portal — surface gets sucked into / blasted out of a
// colored vortex. Composed of a radial-gradient background, two
// layers of whirled bands, an edge rim, and sparkling specular dots.
//
...
#define uRotationSpeed customParams[0].x
#define uWhirling      customParams[0].y
#define uDetails       customParams[0].z

// metadata.json color slot → customColors[0]. BMW's `uColor` is vec3
...
// `easeInBack`, `easeOutBack`, `whirl`, `darken`, `lighten` live in
// BMW's common.glsl but are not re-exported by `<bmw_compat.glsl>`.
// Defined locally so the body stays 1:1 against upstream.
