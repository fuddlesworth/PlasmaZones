...
vec2 computeInstanceUV(int idx, int totalCount, vec2 globalUV, float aspect, float time,
    ...
    vec2 uv = globalUV;
    float wobbleAmp = pz_logoWobble >= 0.0 ? pz_logoWobble : 0.12;
...
vec4 renderEosZone(vec2 fragCoord, vec4 rect, vec4 fillColor, vec4 borderColor, vec4 params,
    ...
    float speed      = pz_speed >= 0.0 ? pz_speed : 0.10;
    float flowSpeed  = pz_flowSpeed >= 0.0 ? pz_flowSpeed : 0.20;
    float noiseScale = pz_interiorScale >= 0.0 ? pz_interiorScale : 3.5;
    int octaves      = int(pz_octaves >= 0.0 ? pz_octaves : 6.0);
    float gridScale    = pz_orbitRadius >= 0.0 ? pz_orbitRadius : 5.0;
    float gridStrength = pz_lineStrength >= 0.0 ? pz_lineStrength : 0.20;
    float brightness   = pz_brightness >= 0.0 ? pz_brightness : 0.7;
    ...
    float innerGlowStr = pz_innerGlowStrength >= 0.0 ? pz_innerGlowStrength : 0.35;
    float sparkleStr   = pz_sparkleIntensity >= 0.0 ? pz_sparkleIntensity : 2.0;
    float connThreshold = pz_packetDirection >= 0.0 ? pz_packetDirection : 0.25;
    float logoScale     = pz_logoScale >= 0.0 ? pz_logoScale : 0.5;
    float logoIntensity = pz_logoIntensity >= 0.0 ? pz_logoIntensity : 0.75;
    ...
    float gradCenterX = pz_orbitCenterX >= -1.5 ? pz_orbitCenterX : 0.5;
    float gradCenterY = pz_orbitCenterY >= -1.5 ? pz_orbitCenterY : 0.5;
    float gradAngle   = pz_gradientAngle >= 0.0 ? pz_gradientAngle : 0.6;
    ...
    vec3 palPrimary   = colorWithFallback(pz_primaryColor.rgb, EOS_PERI);
    vec3 palSecondary = colorWithFallback(pz_secondaryColor.rgb, EOS_PURPLE);
    vec3 palAccent    = colorWithFallback(pz_accentColor.rgb, EOS_CORAL);
    vec3 palGlow      = colorWithFallback(pz_glowColor.rgb, EOS_GLOW);
