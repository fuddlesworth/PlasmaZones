...
// Voxel Terrain — Buffer Pass (full-screen 3D scene)
//
// DDA voxel raycasting across the entire screen. Camera flies a winding path;
// terrain is carved clear around the path. Audio modulates glow, wireframe,
// emission — geometry stays stable.
//
// Outputs:
//   location 0 — vec4 fragColor (gamma-corrected 3D scene)
//   location 1 — float oDepth   (normalized hit distance, 0=near 1=far/miss)
...
void main() {
    ...
    if (hit.hit) {
        ...
        vec3 fogColVol = hasAudio
            ? mix(primary * 0.6, accent * 0.3, bass * 0.5)
            : mix(primary * 0.4, accent * 0.15, 0.3);
        col += fogColVol * volG * 0.08;
    }
    ...
    // ── Depth output ────────────────────────────────────────
    // Normalized: 0 = near, 1 = far/miss. 60.0 = max ray distance.
    oDepth = hit.hit ? clamp(hit.t / 60.0, 0.0, 1.0) : 1.0;
}
