...
vec4 renderEosZone(vec2 fragCoord, vec4 rect, vec4 fillColor, vec4 borderColor, vec4 params,
    ...
    float gradCenterX = p_orbitCenterX >= -1.5 ? p_orbitCenterX : 0.5;
    float gradCenterY = p_orbitCenterY >= -1.5 ? p_orbitCenterY : 0.5;
    ...
    vec3 palPrimary   = colorWithFallback(p_primaryColor.rgb, EOS_PERI);
    vec3 palSecondary = colorWithFallback(p_secondaryColor.rgb, EOS_PURPLE);
