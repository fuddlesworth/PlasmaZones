...
vec4 renderFedoraZone(vec2 fragCoord, vec4 rect, vec4 fillColor, vec4 borderColor, vec4 params,
    ...
    vec3 palPrimary   = colorWithFallback(p_primaryColor.rgb, FROST_BLUE);
    vec3 palSecondary = colorWithFallback(p_secondaryColor.rgb, FROST_DEEP);
...
vec4 compositeFedoraLabels(vec4 color, vec2 fragCoord,
    ...
    if (labels.a > 0.01) {
        ...
        vec3 tubeColor = triStopPalette(frostWave + time * 0.06, palPrimary, palAccent, palGlow);
