...
//
// Rippled-glass pack, buffer pass 0: HORIZONTAL half of a separable 9-tap
// Gaussian over the BACKDROP capture (the scene behind the window), rendered
// at the pack's bufferScale (quarter res). Sampling goes through
// backdropTexel(), which clamps into the capture's valid sub-rect and
// compiles to transparent on the daemon (no scene there) — the main pass
// falls back to a plain tint on uHasBackdrop. Offsets step in canvas UV:
// blurRadius is logical px, scaled to device px by uSurfaceScale, normalized
// by the canvas extent (uSurfaceSize), and spread over the kernel's 4-tap
// reach.
