...
- SPDX on every shader file. Licensing is split by home: reusable helper homes are LGPL-2.1-or-later (`phosphor/libs/phosphor-shaders/**`, shared easing/noise helpers such as `plasmazones/data/animations/shared/easing.glsl`); pack-specific and other `plasmazones/data/` shaders are GPL-3.0-or-later (e.g. `plasmazones/data/overlays/shared/common.glsl`). Check the sibling files' existing headers before flagging, and flag GPL creeping into an LGPL helper home.
