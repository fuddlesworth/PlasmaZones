...
description: PlasmaZones Wayland protocol reviewer. Use for audit partitions covering phosphor-libs/libs/phosphor-wayland (protocol wrappers, QPA plugin, protocol XML), phosphor-libs/libs/phosphor-layer (layer-shell transport, topology), and other code speaking raw Wayland (wl_*, zwlr_*, ext_*) or Qt Wayland client internals. phosphor-libs/libs/phosphor-protocol is the D-Bus contract library, not Wayland, and belongs to pz-cpp-core-reviewer. Expert in wayland-client object lifetime, layer-shell semantics, protocol versioning, and compositor-loss recovery.
---
...
- These libs are LGPL-2.1-or-later (including their `tests/`); SPDX + `#pragma once` on every header; `PHOSPHORWAYLAND_EXPORT` / `PHOSPHORLAYER_EXPORT` on public API. Each phosphor library defines its own `PHOSPHOR<LIB>_EXPORT`; `PLASMAZONES_EXPORT` appears in zero files outside `plasmazones/src/**`, so writing it into one of these headers names an undefined macro and breaks the build.
