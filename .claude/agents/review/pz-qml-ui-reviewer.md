...
description: PlasmaZones QML/Kirigami UI reviewer. Use for audit partitions covering .qml files and the QML-facing controller layer in src/settings, src/editor, src/shell, src/ui, and kcm. Expert in Qt Quick 6, Kirigami, QQC2 pitfalls, and this repo's settings-page patterns.
---
...
- **Module registration**: every QML file must be listed in `qt6_add_qml_module()` — a new .qml missing from CMake is a runtime "not a type" error; grep the CMakeLists to verify.
- **i18n**: QML uses `i18n()`/`i18nc()` (via PhosphorLocalizedContext). New QML strings never appear in `translations/*.ts` (update-ts is C++-only) — do not flag that as missing extraction.
...
- Translucency in KWin-side QML items goes through `Item::setOpacity`, not QColor alpha (OutlinedBorderItem ignores QColor alpha); zone `activeOpacity` is the sole alpha source and colour alphas must be stripped in Rectangle paths.
