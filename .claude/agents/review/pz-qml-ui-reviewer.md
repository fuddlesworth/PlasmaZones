...
- **i18n**: QML uses `i18n()`/`i18nc()` (via PhosphorLocalizedContext). The update-ts target runs lupdate over QML too, but lupdate extracts only qsTr-family calls, so `i18n()` strings never land in `plasmazones/translations/*.ts` — do not flag that as missing extraction.
