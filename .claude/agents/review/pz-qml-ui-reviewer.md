...
- **i18n**: QML uses `i18n()`/`i18nc()` (via PhosphorLocalizedContext). QML is NOT handed to lupdate directly, because lupdate's QML parser only recognizes the qsTr family. `update-ts` runs `scripts/qml-i18n-stubs.py` first, which transcribes each `i18n()` call into a C++ stub lupdate does understand, and feeds those stubs to lupdate instead (`cmake/PhosphorTranslations.cmake:87-92,120-151`). So QML `i18n()` strings DO reach `plasmazones/translations/*.ts`: the great majority of the i18n-family sources under `plasmazones/src/**/*.qml` are present in the German catalog today. When you measure that, pick the source argument per call form — `i18n(src)`, `i18nc(ctx, src)`, `i18np(src, plural)`, `i18ncp(ctx, src, plural)` — and match the longer names first. A bare `i18n("` grep undercounts badly: it misses `i18nc`'s second argument entirely and mistakes its context string for the source. A new QML `i18n()` string that does not reach the catalogs IS a finding, and an urgent one: `-no-obsolete` means a scrape gap deletes live translations from all seven catalogs in one run. `phosphor-libs/libs/phosphor-control/qml` and `plasmazones/kcm` use `qsTr()` and go to lupdate raw; the phosphor-shell-libs tier is qsTr-only by design.
