...
description: PlasmaZones configuration/settings reviewer. Use for audit partitions covering plasmazones/src/config, plasmazones/src/settings C++ (controllers, Settings, ConfigDefaults, migrations), phosphor-config, phosphor-shortcuts, and rules/profile plumbing. Expert in the ISettings architecture, schema migration policy, and shortcut backends.
---
...
- Chain: `ISettings` interface → `Settings` class → `PhosphorConfig::Store` over a pluggable `PhosphorConfig::IBackend` (default `JsonBackend` at `~/.config/plasmazones/config.json`). Defaults live ONLY in `ConfigDefaults`; the old `.kcfg` schema files were removed from the repo, so a reappearing `.kcfg` is itself a finding.
- **Adding a setting requires all six touchpoints**, in the store-backed shape: `configdefaults_<area>.h` (static default accessor, plus `constexpr` Min/Max for a clamped numeric), `configkeys.h`, or `configkeys_scrolling.h` for a scrolling group (group and key accessors, if new — NOT configdefaults.h, which declares none; the call is still spelled `ConfigDefaults::` via inheritance), `settingsschema*.cpp` (the `{key, default, QMetaType, description, coercion}` KeyDef — the store takes its default, type and clamping from here, not from the getter), `plasmazones/src/core/interfaces/isettings.h` (ISettings signal), `plasmazones/src/config/settings.h` (Q_PROPERTY + getter + setter, **no member variable**), and the matching `plasmazones/src/config/settings/*.cpp` (store-backed getter and setter — there is **no** load/save/reset arm, persistence goes through the store). A partial wiring is a finding. Note a clamped setter must write first and compare after, because the schema's coercion runs on the write.
...
- **Qt6 strings, i18n, QUuid, SPDX/licensing, file-size** rules apply as everywhere: `QLatin1String` for JSON keys, `PhosphorI18n::tr()` in C++, braces except in filesystem paths, GPL for plasmazones/src/**, LGPL for phosphor libs.
