...
- **Adding a setting requires all four touchpoints**: `configdefaults.h` (default + key accessor), `plasmazones/src/core/interfaces/isettings.h` (ISettings signal — ISettings was split out of interfaces.h), `settings.h` (Q_PROPERTY + getter + setter + member), `settings.cpp` (changed-check setter, load/save/reset via `ConfigDefaults::xxx()`). A partial wiring is a finding — verify all four for every setting touched.
