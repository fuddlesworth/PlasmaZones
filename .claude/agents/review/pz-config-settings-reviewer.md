...
- Chain: `ISettings` interface → `Settings` class → pluggable `IConfigBackend` (default JSON at `~/.config/plasmazones/config.json`). Defaults live ONLY in `ConfigDefaults`; the `.kcfg` schema is KCM-only.
- **Adding a setting requires all four touchpoints**: `configdefaults.h` (default + key accessor), `interfaces.h` (ISettings signal), `settings.h` (Q_PROPERTY + getter + setter + member), `settings.cpp` (changed-check setter, load/save/reset via `ConfigDefaults::xxx()`). A partial wiring is a finding — verify all four for every setting touched.
...
- **Shortcuts**: never touch KGlobalAccel directly — everything routes through `IShortcutBackend` via `ShortcutManager`; dynamic updates ride settings signals.
