...
private:
    ...
    int pinPerOutputDesktop()
    {
        ...
        m_layoutManager->setCurrentVirtualDesktopForScreen(m_screenId, desktop);
        return desktop;
    }
...
private Q_SLOTS:
    ...
    // WindowQuery::valueForField reports them ENGAGED-but-empty rather than
    // absent, and a negated leaf therefore matches. That is documented behaviour
    // in buildContextualRuleQuery, not an accident, and the `Equals ""` twin of it
    // is closed at authoring time by MatchExpression::isValid. Pin it so a future
    // change to the stamp's empty-screen branch is a deliberate one.
    void testShouldFloatByRule_unresolvableScreen_negatedLeafMatches()
    {
        auto* registry = new PhosphorEngine::WindowRegistry(m_parent);
        m_wta->setWindowRegistry(registry);
        ...
        // No assignWindowToZone, no engine tracking, no hint — the query carries an
        // empty ActiveLayout.
        const QString windowId = QStringLiteral("nowhereapp|inst13");
        QVERIFY2(m_wta->shouldFloatByRule(windowId),
                 "an unresolvable screen leaves ActiveLayout engaged-but-empty, so a negated leaf matches. That is "
                 "deliberate, and it is why a rule cannot pair an empty literal with Equals");
    }
