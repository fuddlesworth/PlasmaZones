...
void TestRuleController::authoringMetadata()
{
    ...
    // alphabetised (Gaps=0, Layout & engine=1, Overlay=2, Zone selector=3), then
    // the window-domain categories, alphabetised (Animation=4, Appearance=5,
    // Window=6).
    ...
    QCOMPARE(actionCategoryOrder.value(QStringLiteral("setZoneSelectorProperty")), 3); // Zone selector (context)
    QCOMPARE(actionCategoryOrder.value(QStringLiteral("excludeAnimations")), 4); // Animation (window)
    QCOMPARE(actionCategoryOrder.value(QStringLiteral("setOpacity")), 5); // Appearance (window)
    QCOMPARE(actionCategoryOrder.value(QStringLiteral("exclude")), 6); // Window (window)
}
