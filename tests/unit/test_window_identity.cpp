...
/**
 * @brief Reimplementation of isMalformedWindowId for isolated testing
 *
...
private Q_SLOTS:
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Malformed Window ID Detection Tests
    // ═══════════════════════════════════════════════════════════════════════

    void testIsMalformedWindowId_empty()
    {
        QVERIFY(isMalformedWindowId(QString()));
        QVERIFY(isMalformedWindowId(QStringLiteral("")));
    }

    void testIsMalformedWindowId_startsWithSpace()
    {
        // Transient windows with empty class generate IDs like " : :123"
        QVERIFY(isMalformedWindowId(QStringLiteral(" : :123456789")));
        QVERIFY(isMalformedWindowId(QStringLiteral(" :resource:123")));
        QVERIFY(isMalformedWindowId(QStringLiteral(" ::123")));
    }

    void testIsMalformedWindowId_startsWithColon()
    {
        // No window class at all
        QVERIFY(isMalformedWindowId(QStringLiteral(": :123456789")));
        QVERIFY(isMalformedWindowId(QStringLiteral(":resource:123")));
        QVERIFY(isMalformedWindowId(QStringLiteral("::123")));
    }

    void testIsMalformedWindowId_validIds()
    {
        // Valid window IDs should not be detected as malformed
        QVERIFY(!isMalformedWindowId(QStringLiteral("firefox:Navigator:123456789")));
        QVERIFY(!isMalformedWindowId(QStringLiteral("org.kde.dolphin:dolphin:987654321")));
        QVERIFY(!isMalformedWindowId(QStringLiteral("konsole:konsole:111222333")));
        QVERIFY(!isMalformedWindowId(QStringLiteral("simple-app:main:999")));
    }

    void testIsMalformedWindowId_edgeCases()
    {
        // Single colon - should be valid (just class, no resource)
        QVERIFY(!isMalformedWindowId(QStringLiteral("app:123")));

        // Just a colon - malformed
        QVERIFY(isMalformedWindowId(QStringLiteral(":")));

        // Just spaces - malformed
        QVERIFY(isMalformedWindowId(QStringLiteral("   ")));
    }
};
