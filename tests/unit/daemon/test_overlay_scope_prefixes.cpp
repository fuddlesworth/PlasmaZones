 ...
 * per-Role configs against the BASE scope from PRoles
 ...
 * matching PRoles base, the lookup silently falls back to the empty
 ...
 * `PRoles::makePerInstanceRole(base, id, gen)` so the prefix-match is
 ...
 *   where base is one of PRoles::{Osd, ZoneSelector,
...
std::unique_ptr<PAL::SurfaceAnimator> buildAnimatorMatchingDaemon(PhosphorAnimation::PhosphorProfileRegistry& registry)
{
    ...
    anim->registerConfigForRole(PRoles::Osd, osdConfig);
    ...
    const PAL::SurfaceAnimator::Config layoutPickerConfig{.showProfile = QStringLiteral("popup.layoutPicker.show"),
                                                          ...
                                                          .showShaderParameters = {},
                                                          .hideShaderParameters = {}};
    anim->registerConfigForRole(PRoles::LayoutPicker, layoutPickerConfig);
    ...
    const PAL::SurfaceAnimator::Config zoneSelectorConfig{.showProfile = QStringLiteral("popup.zoneSelector.show"),
                                                          ...
                                                          .showShaderParameters = {},
                                                          .hideShaderParameters = {}};
    anim->registerConfigForRole(PRoles::ZoneSelector, zoneSelectorConfig);
    ...
    const PAL::SurfaceAnimator::Config snapAssistConfig{.showProfile = QStringLiteral("popup.snapAssist.show"),
                                                        ...
                                                        .showShaderParameters = {},
                                                        .hideShaderParameters = {}};
    anim->registerConfigForRole(PRoles::SnapAssist, snapAssistConfig);

    return anim;
}

/// Build a per-instance Role the way the daemon does. Routes through the
/// production helper `PRoles::makePerInstanceRole` (used by the OSD,
...
{
    return PlasmaZones::PRoles::makePerInstanceRole(base, screenId, generation);
}
...
private Q_SLOTS:
    ...
    /// rename of any PRoles literal that the daemon's create paths
    /// don't also pick up.
    void p_role_base_scope_prefixes_match_production_families()
    {
        QCOMPARE(PRoles::Osd.scopePrefix, QStringLiteral("plasmazones-osd"));
        QCOMPARE(PRoles::ZoneSelector.scopePrefix, QStringLiteral("plasmazones-zone-selector"));
        QCOMPARE(PRoles::SnapAssist.scopePrefix, QStringLiteral("plasmazones-snap-assist"));
        QCOMPARE(PRoles::LayoutPicker.scopePrefix, QStringLiteral("plasmazones-layout-picker"));
    }
    ...
    /// PRoles::Osd.scopePrefix, configFor returns the empty default
    ...
    void osd_scope_resolves_to_registered_config()
    {
        ...
        const auto role = perInstanceRole(PRoles::Osd, QStringLiteral("DP-1"), 7);
    ...
    void osd_scope_resolves_independent_of_screen_id()
    {
        PhosphorAnimation::PhosphorProfileRegistry registry;
        const auto anim = buildAnimatorMatchingDaemon(registry);
        const auto role = perInstanceRole(PRoles::Osd, QStringLiteral("HDMI-A-1/vs:0"), 3);
        const auto cfg = anim->configForRole(role);
        QCOMPARE(cfg.showProfile, QStringLiteral("osd.show"));
        QCOMPARE(cfg.showScaleProfile, QStringLiteral("osd.show"));
    }
    ...
    void zone_selector_scope_resolves_to_dedicated_paths()
    {
        PhosphorAnimation::PhosphorProfileRegistry registry;
        const auto anim = buildAnimatorMatchingDaemon(registry);
        const auto role = perInstanceRole(PRoles::ZoneSelector, QStringLiteral("DP-1/vs:0"), 12);
        const auto cfg = anim->configForRole(role);
    ...
    void snap_assist_scope_resolves_to_dedicated_path()
    {
        PhosphorAnimation::PhosphorProfileRegistry registry;
        const auto anim = buildAnimatorMatchingDaemon(registry);
        const auto role = perInstanceRole(PRoles::SnapAssist, QStringLiteral("DP-1"), 5);
        const auto cfg = anim->configForRole(role);
    ...
    void layout_picker_scope_resolves_to_dedicated_paths()
    {
        PhosphorAnimation::PhosphorProfileRegistry registry;
        const auto anim = buildAnimatorMatchingDaemon(registry);
        const auto role = perInstanceRole(PRoles::LayoutPicker, QStringLiteral("DP-1"), 1);
        const auto cfg = anim->configForRole(role);
    ...
    void layout_picker_does_not_borrow_osd_paths()
    {
        PhosphorAnimation::PhosphorProfileRegistry registry;
        const auto anim = buildAnimatorMatchingDaemon(registry);
        const auto cfg = anim->configForRole(PRoles::LayoutPicker);
    ...
    void osd_does_not_cross_pollinate_to_layout_picker()
    {
        PhosphorAnimation::PhosphorProfileRegistry registry;
        const auto anim = buildAnimatorMatchingDaemon(registry);
        const auto role = perInstanceRole(PRoles::Osd, QStringLiteral("DP-1"), 1);
        const auto cfg = anim->configForRole(role);
    ...
    void exact_base_scope_resolves()
    {
        PhosphorAnimation::PhosphorProfileRegistry registry;
        const auto anim = buildAnimatorMatchingDaemon(registry);
        QCOMPARE(anim->configForRole(PRoles::Osd).showProfile, QStringLiteral("osd.show"));
        QCOMPARE(anim->configForRole(PRoles::ZoneSelector).showProfile, QStringLiteral("popup.zoneSelector.show"));
    }
    ...
    void unregistered_role_falls_back_to_default()
    {
        ...
        const auto cfg = anim->configForRole(PRoles::ShaderPreview);
