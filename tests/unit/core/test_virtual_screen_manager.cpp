 ...
 * @brief Unit tests for PhosphorScreens::ScreenManager virtual screen management methods
 *
...
private Q_SLOTS:
    ...
    void testSetEmptyConfig_clearsSubdivisions()
    {
        ...
        mgr.setVirtualScreenConfig(physId, PhosphorScreens::VirtualScreenConfig{});
        QVERIFY(!mgr.hasVirtualScreens(physId));
    }
    ...
    void testHasVirtualScreens_singleScreen_returnsFalse()
    {
        ...
        config.screens.append(PhosphorScreens::VirtualScreenDef{PhosphorIdentity::VirtualScreenId::make(physId, 0),
                                                                physId, QStringLiteral("Full"), QRectF(0, 0, 1, 1), 0});
    ...
    void testSignal_virtualScreensChanged_emittedOnConfigSet()
    {
        ...
        QSignalSpy spy(&mgr, &PhosphorScreens::ScreenManager::virtualScreensChanged);
        mgr.setVirtualScreenConfig(physId, makeSplitConfig(physId));
    ...
    void testSignal_regionEdit_firesRegionsChangedOnly()
    {
        ...
        QSignalSpy topologySpy(&mgr, &PhosphorScreens::ScreenManager::virtualScreensChanged);
        QSignalSpy regionsSpy(&mgr, &PhosphorScreens::ScreenManager::virtualScreenRegionsChanged);

        // Same ids and display names, just a different split point.
        PhosphorScreens::VirtualScreenConfig edited;
    ...
    void testSignal_displayNameOnly_firesVirtualScreensChanged()
    {
        ...
        PhosphorScreens::VirtualScreenConfig renamed;
    ...
    void testMultiplePhysicalScreens_independentConfigs()
    {
        ...
        mgr.setVirtualScreenConfig(physA, PhosphorScreens::VirtualScreenConfig{});
        QVERIFY(!mgr.hasVirtualScreens(physA));
        QVERIFY(mgr.hasVirtualScreens(physB));
    }
    ...
    void testSetVirtualScreenConfig_overlapping_rejected()
    {
        ...
        // share a 0.1-wide overlap band, well above PhosphorScreens::VirtualScreenDef::Tolerance.
        PhosphorScreens::VirtualScreenConfig config;
        config.physicalScreenId = physId;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // refreshVirtualConfigs — Settings → PhosphorScreens::ScreenManager observer pattern
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    void testRefreshVirtualConfigs_addsNewEntries()
    {
        ...
        QHash<QString, PhosphorScreens::VirtualScreenConfig> configs;
    ...
    void testRefreshVirtualConfigs_emissionsBeforeConnectAreNotReplayed()
    {
        ...
        QHash<QString, PhosphorScreens::VirtualScreenConfig> configs{{physId, makeSplitConfig(physId)}};
        mgr.refreshVirtualConfigs(configs);
        QVERIFY(mgr.hasVirtualScreens(physId));
        ...
        // signal semantics or has wired PhosphorScreens::ScreenManager to replay state on
    ...
    void testRefreshVirtualConfigs_createRemoveRecreateCycle()
    {
        ...
        {
            QSignalSpy spy(&mgr, &PhosphorScreens::ScreenManager::virtualScreensChanged);
            QHash<QString, PhosphorScreens::VirtualScreenConfig> configs{{physId, config}};
            mgr.refreshVirtualConfigs(configs);
            QCOMPARE(spy.count(), 1);
            QVERIFY(mgr.hasVirtualScreens(physId));
            QCOMPARE(mgr.virtualScreenIdsFor(physId).size(), 2);
