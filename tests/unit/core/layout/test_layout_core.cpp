...
private Q_SLOTS:
    ...
    void testLayout_dirtyTracking_setterMarksDirty()
    {
        ...
        // The three visibility allow-lists. Every setter above is generated by
        // the LAYOUT_SETTER macros, so none of them can lose its dirty mark
        // individually — these three are hand-written, which makes dropping
        // one emitModifiedIfNotBatched() a plausible single-line edit, and the
        // consequence is that a visibility change never reaches saveLayout.
        layout.clearDirty();
        layout.setAllowedScreens({QStringLiteral("DP-1")});
        QVERIFY2(layout.isDirty(), "setAllowedScreens did not mark the layout modified");

        layout.clearDirty();
        layout.setAllowedDesktops({2});
        QVERIFY2(layout.isDirty(), "setAllowedDesktops did not mark the layout modified");

        layout.clearDirty();
        layout.setAllowedActivities({QStringLiteral("{activity-uuid}")});
        QVERIFY2(layout.isDirty(), "setAllowedActivities did not mark the layout modified");
    }
    ...
    void testLayout_clone_copiesAspectRatio()
    {
        ...
        // The visibility allow-lists ride the clone too. Dropping one of those
        // three copies is a silent one-line loss: the clone becomes visible
        // everywhere rather than failing, so nothing else in the suite notices.
        original.setAllowedScreens({QStringLiteral("DP-1")});
        original.setAllowedDesktops({2, 3});
        original.setAllowedActivities({QStringLiteral("{activity-uuid}")});

        const std::unique_ptr<PhosphorZones::Layout> copy(original.clone());
        QCOMPARE(copy->aspectRatioClass(), AspectRatioClass::Portrait);
        QCOMPARE(copy->minAspectRatio(), 0.3);
        QCOMPARE(copy->maxAspectRatio(), 0.9);
        QCOMPARE(copy->allowedScreens(), QStringList{QStringLiteral("DP-1")});
        QCOMPARE(copy->allowedDesktops(), (QList<int>{2, 3}));
        QCOMPARE(copy->allowedActivities(), QStringList{QStringLiteral("{activity-uuid}")});
    }
    ...
    void testLayoutSettings_splitProducesStructuralAndSettings()
    {
        ...
        // The three outer-gap keys that were asserted in neither direction.
        // Without these, dropping any one of them from layoutSettingKeys
        // leaves this file green while that gap silently stops being
        // relocated to the sidecar and rides in the structural half forever.
        QVERIFY(!structural.contains(QStringLiteral("outerGap")));
        QVERIFY(!structural.contains(QStringLiteral("outerGapBottom")));
        QVERIFY(!structural.contains(QStringLiteral("outerGapRight")));
        ...
        // The other three outer-gap keys, asserted in this direction too, so
        // every key in layoutSettingKeys is now pinned both ways.
        QCOMPARE(settings.value(QStringLiteral("outerGap")).toInt(), 12);
        QCOMPARE(settings.value(QStringLiteral("outerGapBottom")).toInt(), 2);
        QCOMPARE(settings.value(QStringLiteral("outerGapRight")).toInt(), 4);
