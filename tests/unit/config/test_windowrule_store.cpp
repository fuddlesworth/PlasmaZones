...
private Q_SLOTS:
    ...
    {
        const PWR::WindowRule a = makeRule(QStringLiteral("DP-1"));
        const PWR::WindowRule b = makeRule(QStringLiteral("HDMI-1"));
    ...
    void testMalformedRule_droppedOnLoad()
    {
        ...
        // valid rule.
        const PWR::WindowRule good = makeRule(QStringLiteral("DP-1"));
        QJsonObject root;
    ...
    {
        // WindowRuleSet::fromJson refuses a non-v4 document — the store
    ...
    void testCanonicalizeOnSave()
    {
        ...
        const PWR::WindowRule rule = makeRule(QStringLiteral("DP-1"));
        QJsonObject root;
        root.insert(QStringLiteral("_version"), 4);
    ...
    {
        WindowRuleStore store(storePath());
        PWR::WindowRule rule = makeRule(QStringLiteral("DP-1"), 310);
        QVERIFY(store.addRule(rule));
        ...
        // Unknown id → rejected.
        PWR::WindowRule unknown = makeRule(QStringLiteral("HDMI-1"));
    ...
    {
        WindowRuleStore store(storePath());
        PWR::WindowRule rule = makeRule(QStringLiteral("DP-1"));
        rule.enabled = true;
        QVERIFY(store.addRule(rule));
...
#include "test_windowrule_store.moc"
