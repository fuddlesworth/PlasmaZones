...
- **Maximizing a scrolling window fills the screen and stays filled**: pressing the maximize button or double-clicking the titlebar on a scrolling monitor gave the window the whole screen for a moment and then shrank it back to its old size in the middle of the screen, so the maximize looked like it had done nothing while the button stayed lit. PlasmaZones remembers the size a window settles on when it will not take the shape of its column, so that it stops renegotiating its size on every scroll step. That memory was being applied to maximized columns too, where the point is that the column covers the whole screen, so the window's own preferred size was handed back to it on the next redraw. Gwenview, Konsole and other applications that pick their own size were the ones affected. A column filling the screen is now left out of that memory, as columns showing a single window and windows in fullscreen already were, and restoring one no longer commits the maximized size to the smaller column it returns to.
