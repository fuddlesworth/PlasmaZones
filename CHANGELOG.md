...
- **The zone picker did not show which zone a window would snap to**: while dragging a window, every zone in a layout card lit up at once instead of only the one under the cursor, so there was no way to tell where the window would land. The zone under the cursor now highlights on its own, and the layouts you are not pointing at keep showing all of their zones as before. The cursor also lands on the right zone more reliably, because the picker used to measure zones against the whole card rather than the smaller preview it draws inside it. That was off by a few pixels for most layouts, and by considerably more for a layout tagged with an aspect ratio that does not match the shape of the preview, such as an ultrawide layout shown in a 16:9 preview ([#780](https://github.com/fuddlesworth/PlasmaZones/pull/780)).
