...
- **Per-window scroll speed**: a Set scroll speed rule multiplies mouse wheel and touchpad scrolling inside matching windows, niri's scroll-factor. Values below 1 slow a window whose own scrolling feels too fast, values above 1 speed up a sluggish one, and everything else on screen keeps its normal speed. Works in Wayland sessions ([#901](https://github.com/fuddlesworth/PlasmaZones/pull/901)).
