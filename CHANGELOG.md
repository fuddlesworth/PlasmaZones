...
- **Tab titles can use a font of your own**: the titles on tab chips had no font of their own and followed the zone label font from Snapping → Overlay appearance, so changing one changed the other. Scrolling → Tabs now has a Font row on the Tab indicator card where you pick the typeface, weight, italic, underline and strikeout for tab titles alone, with a reset button that puts it back to the system font in bold. Titles are now sized to fit the indicator, so the Thickness row decides how big they are and there is no separate size setting ([#938](https://github.com/fuddlesworth/PlasmaZones/pull/938)).
