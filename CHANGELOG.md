...
- **One descriptor-binding table for every shader family**: the overlay, animation, surface and pointer contracts now declare their samplers from a single shared table, in order, with room for eight buffer channels. The pack validator reflects every baked stage against that table and fails a pack whose sampler sits at the wrong slot, which used to surface only as a pipeline failure with nothing naming the cause. Third-party packs that hard-code the old slot numbers for their own samplers need to move them into the consumer range.
