...
- **Tab indicators stay behind the windows in front of them**: a tabbed scrolling column's indicator was drawn above every window on the screen, so a floating window raised over the strip wore a stripe of the column behind it. The indicator now sits above the columns it belongs to and behind anything stacked over the strip, the way it would if it were an ordinary window ([#896](https://github.com/fuddlesworth/PlasmaZones/pull/896)).
- **Tab indicators travel with their columns**: the indicator over a tabbed scrolling column used to disappear for the length of every scroll and reappear once the strip settled. It now slides with the column it labels, on every frame, because the compositor moves it in the same pass that moves the windows. Clicking a tab still activates its window, and the rest of the screen stays click-through ([#890](https://github.com/fuddlesworth/PlasmaZones/pull/890)).
