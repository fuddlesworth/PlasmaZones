...
- **Restore unsnapped window positions on login** ([#606](https://github.com/fuddlesworth/PlasmaZones/pull/606)): genuinely-free and snap-floated windows are now restored to the monitor and position they closed on after a KWin session restore (previously only *snapped* windows were restored cross-screen). A global `restoreUnsnappedWindowsOnLogin` toggle (default **on**, under Snapping → Window → Behavior) plus a per-window `RestorePosition` rule action let specific windows opt in or out.
