...
- **Settings pages show a loading indicator**: opening a heavy settings page for the first time used to leave the viewport empty while the page built in the background, which read as a hang. The settings window now shows a centered spinner with a short caption over that area while the page is still building, and it disappears the moment the page is ready. It only appears after a brief moment of continuous loading, so pages that build quickly never flash it.
