...
- **Appearance → Overlays**: a new section beside Decorations and Animations, laid out the way Decorations is. Appearance and Layouts lead, then a Library holding your saved Overlay Sets and the shader pack browser. Appearance is the overlay's colours, label font, opacity, border and zone numbers; Layouts is which shader each layout draws. When the overlay appears while you drag stays a Snapping setting, on the Overlay page there. ([#1006](https://github.com/fuddlesworth/PlasmaZones/pull/1006))
