...
- **The drag popup shows the scroll strip**: dragging a window to a screen edge on a scrolling screen now opens a strip-mode popup, the scrolling counterpart of the snapping zone selector. Instead of zone layouts it renders the strip itself, one card per column with the tiles stacked inside (or a tab strip for a tabbed column), and dropping on a card's top half prepends the window to that column, the bottom half appends it, a gap between cards opens a new column there, and a tabbed card takes the window as a new tab. An empty strip offers a single drop target that starts the first column. The popup has its own Strip Selector page under Settings → Scrolling with an enable switch, a picker for where the popup appears and how close to the edge it triggers, and the preview size. The position, trigger distance, and preview settings can each be overridden per monitor from the page's scope chips. A new Show the drag selector rule action layers over the enable switch in both directions, so a rule can keep the popup off a recording monitor or offer it there while the global switch is off ([#915](https://github.com/fuddlesworth/PlasmaZones/pull/915)).
...
[Unreleased]: https://github.com/fuddlesworth/PlasmaZones/compare/v3.3.6...HEAD
