...
- **Six blur packs gain brightness, contrast, saturation, vibrancy and noise controls**: no blur pack offered the whole set before, and most offered only a tint, so a blurred pane could not be lifted, muted or dithered the way a stock blur-behind effect allows. Blur, Glass, Frosted Glass, Rippled Glass, Rain on Glass and Phosphor Glass now expose the set, and Duotone gains the noise beside the contrast it already had. Mosaic takes none of them, being a pixel mosaic rather than a blur. Every pack defaults to the look it already had, so nothing changes until you move a control. Vibrancy comes from Hyprland and lifts saturation mostly where a colour is already fairly saturated and bright, so a muted wallpaper stays muted. The noise is a fine grain over the blur that hides banding in smooth gradients. Frosted Glass keeps its own Voronoi grain instead of taking the shared one, and Glass keeps a grain tuned to a driver-dependent hash, so those two look slightly different from machine to machine where the rest do not. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
...
- **The blur-family packs blur with a dual Kawase pyramid**: blur, glass, frosted glass, rippled glass, rain glass, duotone and phosphor glass used a two-pass Gaussian at quarter resolution, which was smooth but could not reach far. They now run a four-level pyramid down to a thirty-second of the canvas and back, the same shape stock blur effects use, so a radius up to 256 px stays smooth. A small radius uses fewer of those levels, with the passes above the level it stops at handing their input straight through. The reach was recalibrated at the same time so that a radius reads as the width it names, so a pack at its old default is a close approximation of how it looked rather than a match. Under the hood any shader pack may now declare up to eight buffer passes, not just a surface one, and a surface pack can give each pass its own resolution through a new `bufferScales` list that the daemon and the KWin effect both honour. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
...
- **The shared overlay and surface multipass headers gain a `channelSize` helper**: a pack that sampled a buffer pass had to size it itself, and the four sizes the uniform block carries stopped covering the eight channels a chain can now run. The helper answers a channel's pixel size from the block where it has one and from the texture where it does not. It is a new public name in a header third-party packs include, so a pack that already defines a function called `channelSize` will fail to compile against this release and needs to rename its own. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
...
- **A decoration pack installed in the user data directory compiled against the system copy of the shared shader headers**: the KWin effect let a user pack win over a bundled one of the same name, but resolved that pack's `#include <surface_*.glsl>` lines from the system directory first. A user pack that shipped its own shared headers, or a development tree linked into the user directory, got its body from one tree and its contract from another and failed to compile whenever the two disagreed. The effect now resolves includes from the pack's own shared directory first, then the user directory, then the system one, which is the order the pack validator already used. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
