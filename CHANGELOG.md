...
- **Virtual screens for ultrawide monitors**: physical monitors can now be subdivided into independent logical screens with their own layouts, autotile state, snap-assist, OSDs, shortcuts, and per-desktop / per-activity assignments. New `VirtualScreenSwapper` exposes swap and rotate D-Bus methods and global shortcuts; a monitor-level OSD acknowledges the swap/rotate action so the user sees which regions moved. The layout editor and settings app are VS-aware end-to-end. Settings UI for virtual-screen configuration moved from the editor into the settings app.
- **Animation App Rules — per-app animation overrides**: new "Animations → App Rules" page where users assign motion curves, shader overrides, and per-event timings to specific window classes. The resolver cascades in the kwin-effect (global → app-rule → window-filter), and the editor supports curve picking, shader-parameter overrides, lock + randomize affordances, and window-class filtering with `Disable animations` short-circuit. Two-layer resolver with leaf seeds gives widgets distinctive per-event tunings even when they share a parent rule.
- **Unified `PassiveOverlayShell`**: OSDs, snap-assist, the layout picker, the zone selector, and the main zone overlay all run inside one shell surface instead of five separate layer-shell windows. The shell is click-through except when a modal popup is up, hides itself when no slot is visible so clicks pass through, releases its input region after every slot show, and enforces explicit z-order (OSDs above popups above zones). Replaces the legacy `NotificationOverlay.qml` lifecycle. Slots animate independently — a snap-assist show no longer cancels an in-flight OSD.
- **Krohnkite-style autotile — drag-insert reorder + unlimited stack**: dragging a tiled window over another now inserts/swaps it into the target slot live (new `Reorder` drag behavior), and a new `Unlimited` overflow behavior lets the stack grow past the algorithm's natural cap instead of evicting windows. Drag-insert has always-active and toggle modes matching the snap trigger, and the dropdowns are exposed in Settings → Tiling → Behavior.
- **Animation engine overhaul + 47 ported shader transitions**: window transitions now run through a new `OffscreenEffect` redirect path with vertex-shader plumbing, parent-FBO bounds extent (`boundsExtent: parent` lets vert shaders span beyond the surface), live-texture sampling of the real rendered surface, and direction-aware leg signals. Ships with compatibility shims that port **28 niri animation shaders** (`niri_compat`) and **9 Burn-My-Windows shaders** (`bmw_compat`) without modification, plus **10 new BMW-inspired transitions**, **`neon-city`** (audio-reactive cityscape), a Matrix-rain port, a 3D `chrome-protocol` rewrite, and user-texture support for image-driven effects. OSDs use surface sampling for morph / slide / slidefade / popin.
- **Animations settings UI overhaul**: eight-phase rework — new `AnimationsPageController`, drill-down sub-pages for OSDs / overlays / shaders / motion sets / app rules / widgets / windows / side panels, per-event shader picker with a generic shader browser, **motion sets** (themed override bundles users can switch between), timing-mode toggle on the General page, change-detection on save, and a shader summary card. A separate Snapping → Shaders page exposes the same browser for snap-assist / zone-selector / layout-picker effects.
- **What's New dialog**: settings app now ships a "What's New" dialog driven by `data/whatsnew.json` showing per-release highlights, opened on first launch after an upgrade or on demand from the About page.
- **Layout ordering for cycling**: the cycle shortcut (and zone-selector cycling) now follows a user-configurable order set per-mode in Settings → Snapping → Ordering and Settings → Tiling → Ordering, instead of the implicit save-time order.
- **Per-mode disable lists for monitors, desktops, and activities**: snapping and autotiling each have their own exclusion lists for specific monitors / virtual desktops / activities, replacing the single global disable. The disabled-context OSD explains which exclusion is active so the user can find the toggle.
- **Native-feeling wheel scroll across every settings page**: every `Flickable` settings page now uses `Kirigami.WheelHandler` for Plasma-aware scroll speed matching the rest of System Settings, while preserving the Flickable drag-to-flick path.
- **Phosphor library SDK extraction**: the daemon, engine, layout API, animation pipeline, screen / workspace / placement services, layer-shell QPA bits, and tile / zone primitives now live in reusable LGPL libraries (`phosphor-engine`, `phosphor-layout-api`, `phosphor-animation`, `phosphor-workspaces`, `phosphor-screens`, `phosphor-placement`, `phosphor-tiles`, `phosphor-zones`, `phosphor-snap-engine`, `phosphor-tile-engine`, `phosphor-config`, `phosphor-wayland`, `phosphor-layer`). A new `phosphor-compositor` SDK adds `DaemonClient` plus handler interfaces so third-party Wayland compositors (e.g. river) can host the daemon's placement / tiling / overlay services without depending on KWin.
- **New D-Bus methods**: `setPerScreenSettings` accepts a batched per-screen settings map so the editor and KCM can apply screen-scoped changes in one round-trip; `requestRunningWindows` + `runningWindowsAvailable` give the App Rules editor an async window picker that doesn't block while the daemon enumerates KWin clients.
...
- **Global "auto-assign new windows for all layouts" toggle** ([#370](https://github.com/fuddlesworth/PlasmaZones/issues/370)): a master switch in Snapping Behavior → Window Handling that forces auto-assign on for every manual layout, regardless of the per-layout icon. Effective behavior is `globalToggle OR layout.autoAssign`, so the per-layout icon still works on its own when the global is off. Default is off, preserving existing semantics on upgrade. While the global is on, the per-layout toggle is disabled and the "Auto/Manual" badge (Layouts grid, layout combo, zone selector, layout picker, layout OSD) shows the effective on state, so the displayed state always matches actual snap behavior.
- **Separate "Desktop switch OSD" toggle** ([#372](https://github.com/fuddlesworth/PlasmaZones/issues/372), [#373](https://github.com/fuddlesworth/PlasmaZones/pull/373)): the layout-preview OSD that fired on virtual-desktop change, activity change, and daemon startup is now governed by its own setting (`Snapping.Effects/OsdOnDesktopSwitch`), independent of `OsdOnLayoutSwitch`. Manual layout switches (cycle / quick-layout shortcut / zone-selector drop / KCM Apply) continue to gate on the existing layout-switch toggle. Default is `true` to preserve current behavior. **Upgrade note**: users who previously set `OsdOnLayoutSwitch=false` purely to silence the startup / VD-switch flash will see those OSDs return on first launch under the new schema — they should toggle "Desktop switch OSD" off in Settings → On-Screen Display to restore the old quiet behavior.

### Changed

- **KCM Apply now respects the layout-switch OSD toggle**: the per-screen preview OSDs emitted after a System Settings → Apply previously fired unconditionally, ignoring the user's `OsdOnLayoutSwitch` setting. They now gate on the same flag as cycle / quick-layout / zone-selector drop. The locked-context OSD remains unconditional — it explains why a requested change had no visible effect on a locked screen, the same pattern used by the mode-toggle locked-feedback path.
...
- **Snap-assist showed occupied windows from other virtual desktops** ([#323](https://github.com/fuddlesworth/PlasmaZones/issues/323)): the drag-path occupancy filter walked the live window list without scoping to the current virtual desktop, so a zone that was actually empty on the user's desktop showed as occupied because a window from another desktop matched it. Now filtered to the current desktop before the occupancy check.
- **Per-activity layout assignments could be overridden by per-monitor defaults**: in the layout registry, the fallback chain resolved monitor defaults before per-activity overrides, so a user-set activity-scoped layout could lose to a monitor default that pre-dated it. Activity assignments now win in the precedence order, matching what the assignments UI implies.
- **Layouts context menu eventually stopped opening**: in the Layouts page, the right-click context menu would silently fail to open after enough show/hide cycles because separator visibility was imperatively flipped on each show, eventually leaving a dead `null`-context binding. Separator visibility is now declarative and the dead null-context guard was removed.
