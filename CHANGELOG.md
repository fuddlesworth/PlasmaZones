...
- **A second window of an app opened at the size of the first one's zone**: snap a Dolphin, Ark or Okular window into a zone, then open another window of the same app. It appeared floating, but as big as the zone, and every further window did the same until the first one was floated or closed. KDE apps save their window size to their own settings whenever they are resized, and a snap is a resize, so the app brought the zone's size back for each new window. PlasmaZones did not step in because it treated a window with a same-app sibling as one to leave alone, and so never floated it, never tracked it until it was focused and never gave it a size. A new window of an app that has a snapped window now opens floating at the size the snapped window had before its snap, where the compositor placed it. The same applies to a window reopened with "Restore windows to their previous zone" turned off. ([#1106](https://github.com/fuddlesworth/PlasmaZones/discussions/1106))
