...
- **Keep one app out of zone restore**: a new quick-start template, "Don't restore an app to its zone", opts one application out of the Restore windows to their previous zone behavior while every other window keeps restoring, which suits a browser that hops between a work zone and a media zone ([#889](https://github.com/fuddlesworth/PlasmaZones/discussions/889), [#901](https://github.com/fuddlesworth/PlasmaZones/pull/901)). The rule action behind it already existed but was labeled "Restore to zone on login", which undersold it. It is now named "Restore to previous zone" after the setting it overrides, and its description spells out that it covers reopens during the session as well as after a logout.
...
- **The rule action picker is organized by what an action affects**: the picker's Window entry used to be a long flat list with three small submenus tacked on the end, while Animation and Appearance sat beside it as separate top-level entries even though everything in them also applies to one window. Everything that acts on a window is now grouped under Window, in named submenus for Placement, Scrolling, Appearance, Animation, Behavior, and the two indicator colour families, so the second level means the same thing everywhere in it. The border and title-bar actions join the opacity and tint ones under Appearance, because the old split left you guessing which of two neighbouring entries held a given colour. Behavior is new and holds the window layer and the per-window scroll speed, which are neither placement nor looks. The divider between the entries that apply to a screen or desktop and the ones that apply to a window now falls in the one place it can ([#947](https://github.com/fuddlesworth/PlasmaZones/pull/947)).
- **Fewer, more useful quick starts in the New Rule dialog**: the quick-start grid ran to five rows, and a third of it was one-action examples that the action picker already offers directly. It is down to eight tiles, each of which either sets up two actions that belong together or is one of the rules people open the dialog to write. Gone are the layout lock, which is the rule form of a shortcut you already have, the portrait-monitor example, the per-app zone-restore override, per-app decoration removal, and the small-window animation skip. Every one of those is still available from the action picker. Exclude an app from placement also gets an icon of its own, because three tiles shared one generic red X and that block of the grid read as the same card three times ([#947](https://github.com/fuddlesworth/PlasmaZones/pull/947)).
