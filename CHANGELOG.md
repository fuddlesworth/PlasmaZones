...
- **Every blur-family decoration has brightness, contrast, saturation, vibrancy and grain controls**: no blur pack offered the whole set before, and most offered only a tint, so a blurred pane could not be lifted, muted or dithered the way a stock blur-behind effect allows. Blur, glass, frosted glass, rippled glass, rain glass and phosphor glass now expose the set, and duotone gains the grain. Every one of them defaults to the look the pack already had, so nothing changes until you move a control. Vibrancy is the Hyprland control: it boosts saturation only where a colour is already saturated and bright, so a muted wallpaper stays muted. The grain is a fine noise over the blur that hides banding in smooth gradients, and it is the same on every GPU.
- **Glass refraction gains the controls Better Blur's refraction has**: a bevel curve that shapes how the bend falls off from the rim, a concave lens mode that shrinks the whole backdrop toward the pane's centre like a thick slab, and an edge mirror that folds the backdrop back inside the pane where a strong bend reaches past its edge, instead of stretching the edge pixel. The edge mirror is also on rippled glass and rain glass, the two other packs that bend the backdrop.
- **Blur presets for the looks people compare against**: the Blur pack ships three presets. "Plasma" matches KWin's default blur strength, "Better Blur" matches Better Blur DX's defaults with its wide radius and lifted saturation, and "Hyprland" uses Hyprland's default brightness, contrast, noise and vibrancy.
- **Backdrop panes can round their top corners only, and their edge feather is adjustable**: every blur, glass, duotone and mosaic pack gained a "Round bottom corners" switch, for a pane that only rounds under a title bar, and an "Edge softness" value for the anti-alias feather of the pane edge.

### Changed

- **The blur-family packs blur with a dual Kawase pyramid**: blur, glass, frosted glass, rippled glass, rain glass, duotone and phosphor glass used a two-pass Gaussian at quarter resolution, which was smooth but could not reach far. They now run a four-level pyramid down to a thirty-second of the canvas and back, the same shape stock blur effects use, so a radius up to 256 px stays smooth. A small radius uses a shallow pyramid, so the work still tracks the radius you ask for. The reach was recalibrated at the same time so that a radius reads as the width it names, which means a pack at its old default looks close to how it did rather than identical. Under the hood a surface pack may now declare up to eight buffer passes, each at its own resolution through a new `bufferScales` list, and the daemon and the KWin effect both honour it.
...
- **The blur radius of every blur-family pack goes up to 256 px**: the cap was 64 px, well short of the wide, soft blur other blur effects reach at their default strength.
- **One descriptor-binding table for every shader family**: the overlay, animation, surface and pointer contracts now declare their samplers from a single shared table, in order, with room for eight buffer channels. The pack validator reflects every baked stage against that table and fails a pack whose sampler sits at the wrong slot, which used to surface only as a pipeline failure with nothing naming the cause. A pack can only use the samplers the contract defines. One a pack declared for itself was never bound by anything on either host, and the validator now says so rather than passing it.

### Fixed

- **A decoration pack installed in the user data directory compiled against the system copy of the shared shader headers**: the KWin effect let a user pack win over a bundled one of the same name, but resolved that pack's `#include <surface_*.glsl>` lines from the system directory first. A user pack that shipped its own shared headers, or a development tree linked into the user directory, got its body from one tree and its contract from another and failed to compile whenever the two disagreed. The effect now resolves includes from the pack's own shared directory first, then the user directory, then the system one, which is the order the pack validator already used.
