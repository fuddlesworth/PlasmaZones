...
- **Zone colours apply in six more overlays**: Berry Drift, Mosaic Pulse, Spectrum Bloom, Spectrum Pulse, Toxic Circuit, and Voxel Terrain ignored the fill or border colour set for a zone, so changing it did nothing in those packs. They now tint with it the way the other overlays do ([#841](https://github.com/fuddlesworth/PlasmaZones/pull/841)).
...
- **Overlay glows keep their proportions on a scaled display**: in about a dozen packs the glow reach, edge fade, and travelling rings were measured in raw pixels while the border and corner beside them followed the display scale. On a HiDPI screen the parts drifted apart, and Berry Drift cut its glow off mid-gradient and left a ring ([#841](https://github.com/fuddlesworth/PlasmaZones/pull/841)).
- **Settings that did nothing are gone**: EndeavourOS Drift showed an Orbit Radius and a Connection Threshold slider, and Chrome Protocol showed two sliders labelled "Reserved". None of them were read by anything ([#841](https://github.com/fuddlesworth/PlasmaZones/pull/841)).
- **Four overlay descriptions match what they draw**: Fedora Drift described neon tubes against a northern-lights sky when it draws frosted glass over data streams, NixOS Drift described nebula pulse borders when its border is a wave of lighting hexagons, and Sonic Ripple and Spectrum Bloom both said their pattern is centred on the zone when it is centred on the screen ([#841](https://github.com/fuddlesworth/PlasmaZones/pull/841)).
