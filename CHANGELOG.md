...
- **Peek at Desktop closed by itself**: with focus follows mouse enabled, KDE's Peek at Desktop ended the moment the cursor moved. Peeked windows are invisible but keep their position, so focus follows mouse kept activating the hidden window under the cursor, and KWin ends the peek whenever a hidden window is activated. Focus follows mouse now pauses while the desktop is showing, automatic refocusing after a retile or unfloat waits too, and a desktop switch animation started during a peek no longer bakes the hidden windows into its captured frame ([#775](https://github.com/fuddlesworth/PlasmaZones/pull/775)).
