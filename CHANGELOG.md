...
- **Drag a window back into the scroll strip**: dragging a window while a screen is in Scrolling mode now re-inserts it where you drop it, the same way dragging works in Snapping and Tiling. The window leaves the strip when the drag starts, so the columns behind it stay still while you aim, and the new position is applied on drop. Dropping between two columns opens a column of its own, dropping onto a column joins it at the row you are over, and pressing Escape puts the window back exactly where it came from. Dragging towards either edge of the screen scrolls the strip so you can reach a column that is off screen. It uses the same trigger settings as the other modes, so you can require a modifier or a mouse button, or leave it always on ([#873](https://github.com/fuddlesworth/PlasmaZones/pull/873)).
...
- **The drop indicator slides cleanly while the strip auto-scrolls**: dragging into the edge of the screen scrolls the strip, and the indicator used to stretch and shrink as it moved instead of simply following the columns. Its transitions are meant to make a new drop target legible, and were being restarted every frame by the scroll. They now run for a real target change and stay out of the way while the scroll is driving ([#873](https://github.com/fuddlesworth/PlasmaZones/pull/873)).
