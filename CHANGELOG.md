...
- **Four overlay descriptions match what they draw**: Fedora Drift described neon tubes against a northern-lights sky when it draws frosted glass over data streams, NixOS Drift described nebula pulse borders when its border is a wave of lighting hexagons, Sonic Ripple said its rings start at the zone centre when they start at the middle of the screen, and Spectrum Bloom said the zone boundary itself morphs when the contour is only seen through the zone ([#841](https://github.com/fuddlesworth/PlasmaZones/pull/841)).
