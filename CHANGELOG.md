...
- **The window height shortcuts no longer claim to resize a tabbed column**: when a column shows its windows as tabs, every tab is drawn at the full height of the column, so there is no per-window height to change. Increase Window Height and Decrease Window Height still accepted the key, reported that they had resized something and relaid the strip out, while nothing on screen moved. Holding the key repeated that for as long as you held it. Both shortcuts now decline on a tabbed column and say so, the same way they do when there is no window to resize. Cycle Window Height Preset and Cycle Window Height Preset Back decline there as well, because they step to the height above or below what the window is drawn at, and on a tabbed column that is the whole column however the presets are set.
