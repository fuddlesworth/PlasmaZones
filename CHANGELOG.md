...
- **Scroll the strip without moving focus**: the strip only ever moved as a side effect of something else, so looking at a column further along meant focusing it first. Hold Meta+Shift and scroll the wheel to move the view along the strip one step per notch while focus stays on the window you were working in. It mirrors Meta+wheel, which moves focus one column per notch, and shares its on/off switch and its invert option. A new View scroll step slider under Scrolling → Windows sets how far a notch goes as a share of the screen. For the keyboard, Meta+Alt+Y and Meta+Alt+Shift+Y scroll a whole screen at a time. The view stays where you scrolled it until you move the focus or use one of the centering shortcuts, so on a screen set to always center the focused column the strip no longer springs back the moment you let go. The tab indicators and the strip's own animation follow the scroll the same way they follow everything else.
...
- **The strip stays where a drag scrolled it**: dragging a window against the edge of a scrolling screen scrolls the strip so you can drop it past the columns on show. Letting go somewhere else, or cancelling the drag, put the strip back where it started, because the view is worked out from the focused column and every layout pass re-derived it. The view now holds where the drag left it, the same way the new scroll shortcuts hold.
