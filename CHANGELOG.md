...
- **A pack that read the blurred backdrop from two passes at different resolutions under-sampled the finer one**: the scene behind a pane is captured once, at the resolution the pack needs, and the resolution was taken from the first pass that read it instead of the densest. A later, sharper pass then sampled texels that were never captured. No bundled pack reads the backdrop from more than one pass, so this was reachable only by a third-party pack. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
