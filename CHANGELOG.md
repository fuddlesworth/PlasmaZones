...
- **A decoration pack looks the same in the browser as in its detail preview**: the browser card and the detail preview drew a pack on surfaces of different sizes, and a pack's blur runs in a buffer sized from the surface it is drawn on, so one pack came out as two different effects. Every preview is drawn at one size now and scaled to fit where it is shown, which makes the card a smaller copy of the detail preview rather than a different picture ([#998](https://github.com/fuddlesworth/PlasmaZones/pull/998)).
