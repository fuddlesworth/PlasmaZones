...
- **Tab labels no longer follow the zone label font**: the labels on a scrolling column's tab chips used to take their typeface, weight and style from the zone label font under Snapping → Overlay appearance. They now have a Font row of their own on Scrolling → Tabs, described above. If you had customized the zone label font, your tab labels stop following it and go back to the system font in bold, so re-pick the font you want on the new row. The zone label font itself is unchanged and still governs the zone overlay and the layout editor. Tab labels are now sized to fit the indicator, so the Thickness row on the same card decides how big they are and there is no separate size setting. The Font size scale slider beside the zone label font no longer affects tab labels, and it still scales the zone overlay's own labels ([#938](https://github.com/fuddlesworth/PlasmaZones/pull/938)).
