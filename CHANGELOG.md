...
- **Keep one app out of zone restore**: the Restore to previous zone rule action opts one application out of the Restore windows to their previous zone behavior while every other window keeps restoring, which suits a browser that hops between a work zone and a media zone ([#889](https://github.com/fuddlesworth/PlasmaZones/discussions/889), [#901](https://github.com/fuddlesworth/PlasmaZones/pull/901)). The action already existed but was labeled "Restore to zone on login", which undersold it. It is now named "Restore to previous zone" after the setting it overrides, and its description spells out that it covers reopens during the session as well as after a logout.
