...
- **Six blur packs gain brightness, contrast, saturation, vibrancy and noise controls**: no blur pack offered the whole set before, and most offered only a tint, so a blurred pane could not be lifted, muted or dithered the way a stock blur-behind effect allows. Blur, Glass, Frosted Glass, Rippled Glass, Rain on Glass and Phosphor Glass now expose the set, and Duotone gains the noise beside the contrast it already had. Mosaic takes none of them, being a pixel mosaic rather than a blur. Every one of these new controls is neutral at its default, so none of them changes anything until you move it. Vibrancy comes from Hyprland and lifts saturation mostly where a colour is already fairly saturated and bright, so a muted wallpaper stays muted. The noise is a fine grain over the blur that hides banding in smooth gradients. Frosted Glass keeps its own Voronoi grain instead of taking the shared one, and Glass keeps a grain tuned to a driver-dependent hash, so those two look slightly different from machine to machine where the rest do not. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Glass refraction gains the controls Better Blur's refraction has**: a bevel curve that shapes how the bend falls off from the rim, a concave lens mode that draws the backdrop in toward the pane's centre near its edges, the way a thick slab reads, and an edge mirror that folds the backdrop back inside the pane where a strong bend reaches past its edge. Left off, the bend keeps reading the captured scene beside the pane, which can make the edges look see-through. The edge mirror is on Rippled Glass and Rain on Glass, whose ripple and droplet offsets push the sample in whatever direction the surface points, and on Glass, where the concave lens is the one mode that reaches past the pane. Glass's other two modes bend inward, so the mirror has nothing to fold back for them. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Blur presets for the looks people compare against**: the Blur pack ships three presets, named after the effects they are aimed at. "Plasma" is a modest blur close to the stock one, "Better Blur" is a wide radius with lifted saturation, and "Hyprland" sets brightness, contrast, noise, vibrancy, saturation and a 128 px radius. They approximate those looks rather than reproducing any upstream's exact defaults. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Every pack that draws a pane's outline can round or square its bottom corners, and most can soften that edge**: "Round bottom corners" is on by default, and turning it off rounds only the top two, which suits a pane sitting against a panel or the screen edge. Twenty packs carry it now, the eight Blur-category packs plus the ten Borders packs, Glow and Drop Shadow, so every pack in a chain can describe the same shape. "Edge softness" sets the anti-alias feather of the edge, and eighteen of those twenty offer it. The eight Blur packs and nine of the ten Borders packs gained it in this release, Border already had it, and Glow and Drop Shadow take the corner switch alone because a halo's feather already follows its own reach. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123), [#1126](https://github.com/fuddlesworth/PlasmaZones/pull/1126))

### Changed

- **The blur-family packs blur with a dual Kawase pyramid**: blur, glass, frosted glass, rippled glass, rain glass, duotone and phosphor glass used a two-pass Gaussian at quarter resolution, which was smooth but could not reach far. They now run a four-level pyramid down to a thirty-second of the canvas and back, the same shape stock blur effects use, so a radius up to 256 px stays smooth. A small radius uses fewer of those levels, with the passes above the level it stops at handing the matching down level through unchanged. The reach was recalibrated at the same time so that a radius reads as the width it names, so a pack at its old default is a close approximation of how it looked rather than a match. Under the hood an overlay, animation or surface pack may now declare up to eight buffer passes, where the surface cap was four and the animation family had none at all, while the pointer family keeps its own cap of two because its chain runs on every frame the pointer is live, and a surface pack can give each pass its own resolution through a new `bufferScales` list that the daemon and the KWin effect both honour. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Shader packs can ask for smaller buffers**: the smallest render target a pack could declare was an eighth of the canvas on each axis, and it is now a hundred and twenty-eighth. The blur pyramid needs the lower floor so that it keeps halving when you turn the decoration blur scale down, and the bound is shared with the overlay, animation and pointer families, so packs of every kind get it. An installed pack that declared a value below the old floor was quietly clamped up to it and is now honoured as written. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **The shared overlay multipass header gains a `channelSize` helper**: a pack that sampled a buffer pass had to size it itself, and the four sizes the uniform block carries stopped covering the eight channels a chain can now run. The surface family has no equivalent and sizes a later channel with `textureSize` instead, which its own header says. The helper answers a channel's pixel size from the block where it has one and from the texture where it does not. It is a new public name in a header third-party packs include, so a pack that already defines a function called `channelSize` will fail to compile against this release and needs to rename its own. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **The blur radius goes up to 256 px**: the cap was 64 px, well short of the wide, soft blur other blur effects reach at their default strength. This covers the seven Blur-category packs that have a blur radius, which is all of them but Mosaic. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **One descriptor-binding table for every shader family**: the overlay, animation, surface and pointer contracts now take their sampler slots from a single shared table, which has room for eight buffer channels. Each family's header declares the samplers it needs at the slots that table assigns, and how many buffer channels a family actually runs varies. The pack validator reflects every baked stage against that table and fails a pack whose sampler sits at the wrong slot, which used to surface only as a pipeline failure with nothing naming the cause. A pack can only use the samplers the contract defines. One a pack declared for itself was never bound by anything on either host, and the validator now says so rather than passing it. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))

### Fixed

- **A border, glow or shadow kept tracing a rounded corner over a pane that had squared it**: a pane's outline is one shape, but each pack decided it alone. Square a backdrop pack's bottom corners against a panel edge and the Border pack went on drawing a rounded corner through empty space, with Glow and Drop Shadow rounding a corner the pane no longer had. Every pack in a chain now follows one answer. Set it on any pack in the chain and the whole chain follows. Where two packs have it set differently, the one earliest in the chain wins. ([#1126](https://github.com/fuddlesworth/PlasmaZones/pull/1126))
- **A border pack rounded a corner you had squared, and erased the pane's own square corner doing it**: a corner radius of 0 means square, and the Borders family drew it as an arc the width of its own band, up to 20 px on a single-line pack and 52 on Double Border, which dilates by its whole stack. Because a border pack clears the window content outside its band, that arc also cut the matching corner off whatever backdrop pack sat under it, so a squared pane came out rounded and clipped. A radius of 0 now stays square at both ends, and the band mitres the corner the way a drawing program would. A radius above 0 is unchanged, where the band still runs from the radius you set out to that radius plus its width. ([#1126](https://github.com/fuddlesworth/PlasmaZones/pull/1126))
- **A glow or drop shadow lingered inside a corner the pane had squared**: the two packs traced the pane's own outline correctly, but the rule that holds their halo out of the window body used one rounded corner for both ends. On a window whose own body is translucent, where the corner radius is wider than the glow or shadow reach, squaring the bottom corners left the halo lying inside them. Both ends now follow the same shape the outline does. ([#1126](https://github.com/fuddlesworth/PlasmaZones/pull/1126))
- **Turning a decoration border off left a faint line where it had been**: a border width of 0 is the lowest setting on eight controls across seven packs, and it has to mean no line. It did not. The width collapsed to nothing and the edge term painted a band about two feathers wide straddling the frame edge, at up to a quarter of the border colour. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **A glass or rippled pane vanished entirely where nothing sat behind it**: with no scene to blur, those packs fall back to a faint tinted slab, and the slab's alpha was scaled by the tint strength. At a tint strength of 0, which is the lowest setting all three packs offer, it drew nothing at all and the pane's shape was not communicated. It now keeps a visibility floor on that fallback only, so a tint strength of 0 still means no tint on a real backdrop. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **The Glass pack's rim light did nothing over most of its range**: Rim strength acted as a ceiling rather than a level, so 0.25 and 1.0 rendered identically and the shipped default already sat inside that dead range. The thickness glint had the matching problem, gated on the same slider and then ignoring it. Both follow the control across its whole range now. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **An unfocused Audio Border was dimmer than its own setting asked for**: the focus cue was applied twice, so the pack rendered at 55% of the alpha its Inactive colour setting specified. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Glass refraction could not be switched off**: at a Refraction strength of 0 the Snell lens still applied its whole displacement, so the control did not do what its description says. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **The Glass pack's glint thinned to a hairline on a scaled display**: its band was held in raw device pixels, so at 200% it stayed the same number of pixels while everything around it doubled. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **A shader pack installed in the user data directory compiled against the system copy of the shared shader headers**: the KWin effect let a user pack win over a bundled one of the same name, but resolved that pack's `#include` lines from the system directory first. A user pack that shipped its own shared headers, or a development tree linked into the user directory, got its body from one tree and its contract from another and failed to compile whenever the two disagreed. Decoration packs were fixed first. Animation packs had the same problem, and had it in the daemon as well as in the KWin effect, so the two processes could compile the same pack against different headers. Both now resolve includes from the pack's own shared directory first, then the user directory, then the system one, which is the order the pack validator already used. The shared `animation.vert` that a pack falls back to when it declares no vertex stage is picked the same way, so a user copy of it is honoured too. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **A window sent to another virtual desktop takes the zone that desktop's layout has for it**: moving a snapped window to another desktop left it at the size and place of the zone it came from, so on a desktop running a different layout it sat in a zone that desktop does not have. PlasmaZones' own "move window to the next desktop" shortcut already put such a window into the matching slot of the destination layout, but the moves it does not drive itself did not, which covers KWin's own Window One Desktop to the Left and Right shortcuts and dragging a window onto another desktop in the overview. Those now land the window in the same slot of the layout the destination desktop runs, and the destination desktop counts it as that zone's occupant. When the destination layout has no slot in that position, a window from the fourth zone of a grid moved onto a two-zone layout for instance, the window goes back to the size it had before it was snapped. A window that was floating, or one on a desktop that tiles, is left alone as before. ([#1104](https://github.com/fuddlesworth/PlasmaZones/discussions/1104))
- **A second window of an app opened at the size of the first one's zone**: snap a Dolphin window into a zone, then open another Dolphin window. It appeared floating, but as big as the zone, and every further window did the same until the first one was floated or closed. KDE apps save their window size to their own settings whenever they are resized, and a snap is a resize, so the app brought the zone's size back for each new window. PlasmaZones did not step in because it treated a window with a same-app sibling as one to leave alone, so it was neither floated nor given a size, and it was not tracked until it was focused. When nothing else places it, a new window of an app that has a snapped window now opens floating at the size the snapped window had before its snap, centred on the spot where the compositor placed it. The same applies to a window reopened with "Restore windows to their previous zone" turned off, and to a floating window reopened with "Restore unsnapped windows to their previous position" turned off. On a tiling or scrolling screen, a window that a rule leaves floating beside a tiled sibling gets the same treatment, since a tile is a resize the app saves too. ([#1106](https://github.com/fuddlesworth/PlasmaZones/discussions/1106))
- **An oversized texture in a shader pack was decoded at full size inside the compositor**: a pack can name an image file for its texture samplers, and the KWin effect read it with no size bound, so a very large PNG or a near-square SVG allocated the whole thing in the compositor's own process. The daemon has held the same files to a 16 MiB budget all along. The effect now applies that budget too, and scales an oversized image down while it decodes rather than after. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Every blur-family pack captured the scene behind a window at four times the resolution it reads**: the scene behind a pane is captured once, and the capture was sized by asking whether a pack mentions the backdrop at all rather than whether it samples it. All eight backdrop packs read the on-or-off flag in their main shader, so all eight got a full-resolution capture even though the only pass that samples it works at a quarter of the canvas. On a 4K window that is about 33 MB of video memory per decorated window, and a full-canvas copy on every frame, where 8 MB and a quarter of the copy would do. The capture is now sized for the pass that actually reads it, at twice that pass's own resolution, which is what its filter expects. That also fixes a shimmer on the blurred backdrop while the scene behind it moved, since the first blur pass had been averaging single pixels into blocks of sixteen. A pack whose main shader samples the backdrop directly, which of the bundled ones is only Mosaic, still gets the full-resolution capture. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **A pack that read the backdrop from two passes at different resolutions under-sampled the finer one**: the capture resolution was taken from the first pass that read it instead of the densest, so a later, sharper pass sampled texels that were never captured. No bundled pack reads the backdrop from more than one pass, so this was reachable only by a third-party pack. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **A decoration's glow could be clipped for the length of an animation that began during a resize**: the enlarged quad that an animating window's outer margin is drawn on was measured from a rect that can briefly still describe the previous frame, and that one measurement was then kept for the whole animation. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **A shader pack asking how big its own texture is got a different answer in the two processes**: `iTextureResolution` is the list of bound texture sizes, and the KWin effect filled it one place to the left of where the daemon does, so a pack's first image landed on the entry that belongs to the window's own content. The effect also never filled that first entry at all, which the daemon does. Both now agree that entry `i` holds the size of `uTexture<i>`, so a pack's first declared image is entry 1 and entry 0 is the window. No bundled pack reads these sizes, so this only affected packs written against the contract. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **The blur quality setting no longer shrinks the captured backdrop, only the blur passes**: it used to scale both, and scaling the capture was wrong. The first pass of the blur pyramid steps a fixed distance measured on the pane, so shrinking what it reads from moved its five samples closer together until, at the lowest quality, they all landed on the same pixel and that pass stopped blurring at all. The capture is now held at the density the pyramid expects whatever the setting says, and the setting goes on scaling the passes themselves, which is where its cost mostly sits. The trade is that Low no longer saves the memory the capture takes, so a decorated window with a blurred backdrop holds a larger texture at that setting than it did before. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **The decoration blur quality setting only applied to windows**: Low, Balanced and High set the resolution the blur passes render at, and only the KWin effect read it. The same pack decorating an on-screen display, a popup or a shell surface always rendered at its declared resolution, so turning the quality down left those looking different from your windows and costing what they always did. Every host that draws a decoration now applies the setting. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Fireflies lost most of its swarm behind the window**: each spark orbited on an oval drawn around the window rather than following the window's outline, and an oval that size runs inside the window over most of its loop, where the pack draws behind the window and hides the spark completely. The swarm read as four blinking arcs at the middles of the edges and nothing at all near the corners. Measured on a 800x500 window, a spark was visible for 132 degrees of its orbit and is now visible for all 360. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Phosphor Motes drew expanding rings at the corners instead of dust**: the coordinate that spaces the dust along the window's edge was constant across each corner, so a speck that lit up there painted a uniform quarter-ring about half a cell thick and blinked as a whole. The same coordinate also made the dust along the top edge a mirror copy of the dust along the left edge. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Drop Shadow and Glow tinted the whole body of a see-through window**: both packs draw only where the surface is not opaque, and that was the only thing keeping them out of the window. On a window whose own body is translucent, rather than the usual opaque one, the halo covered the entire window instead of the margin around it. They now also stop at the edge of their own reach, so what they draw inside the window goes no further in than it reaches out. An opaque window was never affected and still is not. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **The Glass pack's concave lens magnified the backdrop instead of shrinking it**: the mode is described as shrinking the backdrop toward the pane's centre, the way a thick concave slab reads, and it did the opposite. At full strength the pane's edge showed content from 80% of the way out instead of from past the pane, so the middle of the scene was stretched across the whole pane. It compresses a wider view into the pane now. The mode is off by default, so this only changes how it looks for anyone who turned it on. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Frosted Glass stretched its crystals to the shape of the window**: the crystal pattern was laid out in a grid of the same count on both axes, so on a wide window each crystal came out as a long ellipse rather than a crystal, and the same Grain scale setting gave different crystal sizes on different windows. A wider window now gets proportionally more crystals instead of wider ones, which is what a density setting should do. A square window looks exactly as it did. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Phosphor Motes cut its dust off in a straight line**: the twinkling dust field faded out slowly enough that it was still at about a third of its brightness where the drawing room ran out, so it ended in a hard rectangle with specks sliced through the middle. It now fades to nothing just before that edge. Its drifting motes could also be carried sideways out of the drawing room at a high Sway with a low Mote range, which the shipped settings never reached but the sliders allowed. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Glass bent the backdrop further sideways than up and down on a wide window**: the default refraction measured its displacement against each side of the pane separately, so on a 1600x400 pane one Refraction strength bent the scene four times as far left and right as it did up and down. It now bends the same distance in every direction, measured against the pane's shorter side. A square window looks exactly as it did, and a wide one has its sideways bend brought down to match. Turning on Physically based refraction still makes the bend much smaller at the same strength, since that mode measures against the bevel width instead of the pane, and both settings now say so. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123))
- **Rules edited outside the settings app took effect only after a restart**: the daemon never watched `rules.json`, so a rule added by hand, restored from a backup or written by a config import stayed invisible until the daemon was restarted or told to reload over D-Bus. Worse, the daemon's next rule save wrote its stale set back over the edit. The daemon now watches the file and reloads it when another process changes it. ([#1119](https://github.com/fuddlesworth/PlasmaZones/pull/1119))
...
- **The daemon and the KWin effect must be the same build**: the effect now holds a window out of the scrolling strip for its own fullscreen through a new `Scrolling.setWindowFullscreenFloat` call, and the `leaveNativeFullscreenRequested` signal that released fullscreen ahead of the windowed-fullscreen toggle is gone, because a fullscreen window is no longer in the strip for it to find. A mismatched pair refuses to connect rather than misbehaving, so packagers should rebuild and ship both together. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))

### Fixed

- **A window's saved size could be replaced by the size of the screen**: while a window on a scrolling screen was fullscreen, PlasmaZones could record the full screen as the size to give it back when it floats. It now keeps the size it had before. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **The shell's placement map re-read the strip on every step of a drag**: each change on a scrolling strip made the map ask the daemon for the whole strip again, and the daemon laid the strip out again to answer. The map now waits for a burst of changes to settle and asks once. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **Going fullscreen on a scrolling screen showed a floated notification**: when a tiled window went fullscreen by itself, PlasmaZones took it out of the strip the same way a Meta+F float does, so the on-screen notification said the window was floated, said it was tiled again when the window left fullscreen, and its saved free size was put back over the fullscreen picture. The window still leaves the strip for as long as it is fullscreen, but quietly, with no notification and no size change. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
...
- **A glowing square over the system tray**: apps that still use the old style of tray icon, such as Battle.net and other Windows programs run through Wine or Proton, left a small bordered and glowing square sitting on the panel. The helper that shows those icons keeps a tiny hidden window behind each one, and PlasmaZones mistook it for an application window and drew your window decoration on it. PlasmaZones now leaves those helper windows alone, so they are not decorated or animated and are never offered to snapping or tiling. ([#1115](https://github.com/fuddlesworth/PlasmaZones/pull/1115))
- **A game that opened fullscreen was placed on the scrolling strip**: a window that was already fullscreen when it opened was put on the strip like any other window. With "Group windows of the same application as tabs" turned on, which is off by default, a game that shares an app name with its launcher landed as a tab in the launcher's column. The tab bar could show up over the game, and the first time you used a scrolling shortcut the game was pulled out of fullscreen and stretched across the strip, taking the launcher with it. It happened on some launches and not others, which made it look random. A window that is already fullscreen when it opens is now left alone, the same way snapping and tiling screens treat it, and it joins the strip only if it later leaves fullscreen. Seen with World of Warcraft started from Battle.net under Proton. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **The tab bar could draw over a fullscreen window on a scrolling screen**: a tiled window that went fullscreen by itself, such as a game, a video or a browser after F11, could end up with its column's tab bar painted on top of it, and it could slide when the strip scrolled behind it. PlasmaZones stopped treating the window as part of the strip while it was fullscreen, but the next layout update quietly undid that, so it depended on whether anything else on the strip changed in the meantime. The window now leaves the strip for as long as it is fullscreen, which the entry "A window in its own fullscreen still held its column on the strip" below describes, so no tab bar is drawn over it and nothing moves it. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **Scrolling shortcuts pulled a window out of fullscreen**: moving focus along the strip, by shortcut or by the wheel, took any fullscreen window on that screen out of fullscreen first. A game that only runs at one size then fought the column it was given and could leave its whole column stretched across the screen. 3.4.5 made the wheel do this, and 3.4.6 made every keyboard verb that acts on the strip do it, width and height changes included, so that the strip and a window covering it could not drift apart. That is reversed on purpose. A window in its own fullscreen now leaves the strip instead, so there is nothing left to drift, and moving around the strip by shortcut or by the wheel leaves a fullscreen window alone. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **A window in its own fullscreen still held its column on the strip**: a tiled window that went fullscreen by itself kept its column, so the windows beside it did not close up and focus could land on the hidden column. With "Group windows of the same application as tabs" turned on, a game could also sit as a tab inside its launcher's column. The window now leaves the strip for as long as it is fullscreen. When it leaves fullscreen it returns to its slot in its column at the width it had, or to a fresh column if the window it sat beside has closed in the meantime. PlasmaZones' own windowed fullscreen is not affected. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **Windowed fullscreen turned off every decoration on the monitor**: with "Stop while a window is fullscreen" turned on, putting a column into windowed fullscreen removed the decorations from the panels and from everything else on that monitor until you toggled it back. A window in windowed fullscreen stays its column's size and the panels stay on screen, so it no longer counts as fullscreen for that setting. The window itself still has no border while it is held. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **A fullscreen window could come back undecorated after a desktop round trip**: on a scrolling screen, switch away from a desktop while a window on it is fullscreen, come back, and take the window out of fullscreen. PlasmaZones had set the window aside for the desktop switch and never picked it up again, so it stayed out of the strip and without its decoration until you left the desktop and returned once more. A window set aside like that is now picked up again when it leaves fullscreen with its desktop in front, and it returns to its column straight away. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **A restarted effect could save the screen size as a window's free size**: if the KWin effect restarted while a window on a scrolling screen was fullscreen, the first time that window left fullscreen its full-screen frame was written down as the size to give it back when it floats, and a later float made it fill the screen. The frame of a window that has just left fullscreen is never its free size, so it is no longer recorded. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **The shell's placement map could show the wrong mode's cells**: when a screen changed placement mode while the map was still waiting on an answer from the daemon, the answer for the old mode landed on the new one, so a tiling or snapping screen could briefly show a scrolling strip or a set of zones it does not run. A map that is pinned, or whose screen has moved to another mode, now ignores answers meant for the mode it left. Each step of a drag on a scrolling screen also made that screen's other maps re-read their layout, including maps pinned to a snapping or tiling desktop. Those re-reads are now folded together, like the strip re-read is. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **Restoring a minimized window could put a fullscreen window on the strip**: on a scrolling screen, a window brought back from minimize is put back in its column a moment after the restore animation. If the window went fullscreen in that moment, it was placed on the strip anyway, as a full-screen column. The window is now checked again just before it is put back, and a window that has gone fullscreen is left alone until it leaves fullscreen. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **Maximizing one tab maximized the whole tab group**: clicking the maximize button on the tab in front of you, an app maximizing itself, or KWin's own maximize shortcut stretched the entire column, and the column stayed stretched after that window was gone. A game that opened as a tab beside its launcher could leave the launcher filling the screen. When the window that asks is the one in front of the focused column, and that column holds other windows as tabs or in a stack, it is now moved into a column of its own and maximized alone, and it stays in that column when you un-maximize it. A window in front of a column that is not the focused one still maximizes its whole column, as a background tab does. PlasmaZones' Maximize to Screen Edges shortcut, Meta+Alt+M, still maximizes the focused column with all of its tabs. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
...
- **Strip shaders no longer move the wallpaper**: a strip pack assigned under Settings → Appearance → Animations → Motion → Scrolling ran over a capture of the whole screen, so the wallpaper and any window kept below the columns slid along with them on every scroll, when only the columns are meant to move. The pack now displaces the columns and nothing else. The wallpaper and keep-below windows stay still underneath, the same way notifications and panels were already kept sharp on top. A translucent window stays see-through with the still wallpaper showing through it, and a PlasmaZones decoration pack that frosts what is behind a window keeps that frost through the scroll. KDE's own Blur effect does not draw on the columns while the strip is in motion, as before, and the short settle at the end of a scroll now behaves the same way. Strip Chromatic's color fringe at a column edge now blends over the still wallpaper instead of painting a dark band into the gap, and Phosphor Gate's glow lights the columns only, never the desktop between them. ([#1064](https://github.com/fuddlesworth/PlasmaZones/pull/1064))
...
- **The mouse pointer no longer wobbles along with the scrolling strip**: with a strip animation such as Strip Jelly assigned, a software-rendered pointer was painted into the frame the animation warps, so it bent and smeared with the columns on every scroll. The pointer is now kept out of that frame and drawn sharp on top of the animation. ([#1049](https://github.com/fuddlesworth/PlasmaZones/pull/1049))
...
- **Maximizing a scrolling window fills the screen and stays filled**: pressing the maximize button or double-clicking the titlebar on a scrolling monitor gave the window the whole screen for a moment and then shrank it back to its old size in the middle of the screen, so the maximize looked like it had done nothing while the button stayed lit. Applications that pick their own size, such as Gwenview and Konsole, were the ones affected, because PlasmaZones remembers the size those windows settle on and was handing it back to them on the next redraw. A column filling the screen is now left out of that memory, and restoring one leaves the smaller column it returns to at its own size. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **The maximize path can be diagnosed from a support report**: a maximize on a scrolling monitor wrote nothing to the log at any stage, on either side, so a report gathered right after reproducing the problem showed no sign that the button had been pressed. Claiming the request, dispatching it, refusing it and every reason for declining it are now recorded, along with the shape the column ends up with. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **Windowed fullscreen says what it did**: `Meta+Alt+Shift+F` wrote nothing to the log whether it worked or not, and it has two refusals that look exactly like a dead keybinding from the user's side. The commonest one is that focus is on a floating window, which owns no column for the shortcut to act on. Both refusals now give their reason, the toggle records which window it acted on and which way it went, and the compositor records entering, adopting after a restart, reconciling an application's own exit, and releasing. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **The support report keeps the compositor log it was throwing away**: the KWin Effect Logs section kept only journal lines containing the word "plasmazones", which was expected to match every line the effect writes because its log categories all start with it. KWin prints its own log lines without the category, so in practice only the ones that happened to name a PlasmaZones window survived, and on a live session that discarded more than three quarters of them. Worse, a report could say the effect had logged nothing while it was logging heavily, and the archive shipped without its `kwin-effect.log` at all. The whole compositor journal for the window is now kept, under the clearer heading Compositor Logs, still capped at 2000 lines. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **Windowed fullscreen says why it did nothing when a floating window has focus**: pressing `Meta+Alt+Shift+F` while the window you are looking at is floating showed "no windows", which is the message for an empty strip and reads as nonsense when there is a window right in front of you. A floating window belongs to no column, so there is nothing for the shortcut to fill, and the message now says so. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **A maximized scrolling window is drawn where it actually is**: a window that insists on its own size instead of taking the whole screen was drawn in the middle of the screen while it actually sat at the top-left corner, so clicks landed away from what you could see. A column maximized to the screen edges has nothing to centre a window within, because the column already is the whole screen, and neither the drawing nor the placement centres one there now. Windows that pick their own size on X11 are unaffected, since those are genuinely centred when they are placed. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **The support report redacts paths outside your home directory**: home paths were replaced with `~`, but if you point `XDG_CONFIG_HOME` or `XDG_DATA_HOME` somewhere outside your home directory, every config and data path in the report was written out in full, and those paths usually contain your username. They are now replaced with the name of the variable. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **The support report collector no longer stops when `$HOME` is unset**: running the collector from a context with no home directory set, such as a systemd unit without `User=`, ended it with a shell error before it produced anything. It has a warning for exactly that case which could never be reached, because the first thing it did with the home directory came earlier. The warning now prints and the archive is written without home-path redaction, as intended. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **The support archive is readable only by you**: the archive was written with whatever permissions the shell's default allowed, which on most systems means any other account on the machine could read it, and it is written to a shared temporary directory by default. It carries window titles, your window rules and your whole configuration. It is now created owner-only inside a private directory and moved into place, so another account cannot swap in a link and have the archive written somewhere it can read. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **The support report no longer ships files it cannot redact**: pictures and other non-text files in your configuration, such as a shader pack's preview image, were copied into the archive exactly as they were. Image files routinely carry the folder they were made in, which usually contains your username, and nothing was replacing it. Those files are now left out and listed by name instead, and the report says so. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **A half-redacted file can no longer reach the support archive**: three of the captures hid a failure while they were being cleaned of home paths, and the only check afterwards was whether the file was empty. A file that was cut off partway through was kept, with whatever had not been cleaned yet still in it. Each capture is now checked properly and dropped if it cannot be cleaned. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **One unreadable file no longer destroys the whole support archive**: while copying your configuration, a single file that could not be read ended the collector, and the cleanup then deleted everything it had gathered. Nothing was printed to explain it. A file that cannot be staged is now reported and skipped, and the rest of the archive is still written. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **Interrupting the support collector no longer leaves it writing into a deleted folder**: pressing Ctrl+C partway through deleted the staging folder and then carried on, so the run ended in a confusing error instead of stopping. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **Window rule descriptions and machine names are redacted in the support report**: window titles were replaced, but the name you gave a rule was not, and in practice that name quotes the document or site the rule was made for. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **Windows past the tiling limit stay under control on every path**: the fix for windows opening past the limit covered the case where tiling was on for the screen, and a second path was left. A window arriving while the screen was not tiling it kept a stale entry pointing at that screen, which is what made focus-follows-mouse stop and minimized windows hold their slot. Both paths now clean up after themselves, and a refused float repairs the entry instead of refusing forever. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **Unminimizing a window on a full screen no longer thrashes**: the window was unfloated, the screen retiled, and the overflow pass immediately floated it again, so the compositor kept retrying a request that could never succeed. A window with nowhere to land now stays floating and comes back on its own when a slot frees up. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **Small windows keep their remembered position**: any window narrower or shorter than 100 pixels was refused when PlasmaZones tried to remember where it had been floating, so small tool palettes and dialogs came back wherever the compositor happened to put them. The check was measuring visibility rather than which screen the window belonged to. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **A window no longer loses its remembered position when it moves between monitors**: the old position was cleared before the new one was recorded, and several checks in between could stop the recording, leaving the window with nothing to return to for the rest of its life. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
- **Float-back works again for windows that only ever tiled**: the remembered position was looked up under the snapping screen, which a window that has never been snapped does not have, so the lookup could not match anything. ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031))
...
- **The support report summarizes your session instead of dumping it**: the Session State section printed `session.json` word for word, which on a system that had been running a while ran to thousands of lines and buried everything else in the report. It now gives the active layout, the last zone used, how many placements are stored by kind, and the twenty most recent ones, with the full file still in the archive for anyone who wants it. A new Placement Modes section says which mode each screen resolves to and whether the tiling and scrolling engines are on, since the scrolling engine did not appear in the report at all before. The Rules section gains a line per rule ahead of the JSON, the Config section says that only settings you have changed are listed, the report carries the time it was generated, and the archive script lists the files it packed. ([#1022](https://github.com/fuddlesworth/PlasmaZones/pull/1022))
...
- **Steam's game-launch dialog is kept out of placement**: the built-in Steam rule guarded Steam's notification toasts and nothing else, so the small dialog Steam opens while a game starts was placed into a zone or a column as though it were an ordinary window. Nothing about that dialog tells it apart from the main Steam window except that it cannot be resized, so the rule now covers any Steam window with a fixed size. Steam's ordinary windows still place normally and so does every game launched from Steam, including the many that run at a fixed size themselves. A config carrying the previous version of the rule untouched is brought up to date on the next start, and a copy you edited is left as you left it. ([#1014](https://github.com/fuddlesworth/PlasmaZones/pull/1014))
- **The first click on a window on another desktop works**: with more than one virtual desktop on a scrolling monitor, windows on a desktop you had switched away from and come back to were missing from the strip, and the first click on one from the taskbar did nothing while the second brought it in. Two things caused it. An activation the daemon had asked for itself was remembered for ten seconds waiting for the compositor to confirm it, and a desktop switch in between meant the confirmation never came, so the next real click on that window was swallowed as the echo. And a window that had come back on a desktop whose stored scroll position disagreed with it rendered off the side of the screen, where a click on it was refused for already being focused. The wait is now a second, and a click on a window sitting off the screen brings the strip back to it. ([#1005](https://github.com/fuddlesworth/PlasmaZones/pull/1005))
- **Windows that announce themselves a moment late are left alone**: an application can map a window and only then say that it is a dialog, that it stays above others or that it should be skipped by the task switcher. PlasmaZones read the window before those arrived, so a window it should never have touched was placed into a column, given focus and resized. Yakuake's dropdown terminal was placed this way, and because it stores its size as a share of the screen, one capture changed its size for good. Placement now waits a turn for a window to finish describing itself, and a window that turns out to be a dialog, a modal, kept above or skipped by the switcher after that is released and put back where it was. ([#1009](https://github.com/fuddlesworth/PlasmaZones/pull/1009), [#1011](https://github.com/fuddlesworth/PlasmaZones/pull/1011))
- **Coming back to a desktop leaves the strip working**: returning to a desktop sent the compositor nothing at all, because the check that skips sending unchanged positions was comparing against the desktop you had just left. The compositor kept drawing the previous desktop's strip, so the first scroll after a return looked like it did nothing while the second worked. Each desktop's strip now has an identity the compositor can see, so a return says so plainly and the drawing state for the strip you left is dropped rather than reused. ([#1013](https://github.com/fuddlesworth/PlasmaZones/pull/1013))
- **An expanded rule shows the rule you just edited**: on the Rules page, expanding a rule and then editing it left the expanded details showing the rule as it was before the edit, and you had to collapse the row and open it again to see the change. It now updates in place. ([#1016](https://github.com/fuddlesworth/PlasmaZones/pull/1016))
...
- **Debian testing and unstable have an apt repository**: the only Debian package before now was a loose `.deb` attached to each release, built against KDE Neon, whose library versions do not match testing or unstable. Packages for both are published through the openSUSE Build Service instead, which rebuilds them whenever KWin moves in Debian. That matters because the KWin effect stops loading on every KWin version bump until it is rebuilt, taking the drag overlay and the other compositor-drawn feedback with it, and KWin moves every few weeks on those releases. The install lines are in the README, and the same repository works on Debian-derived rolling distributions such as PikaOS. Packages are built for x86_64 only ([#1000](https://github.com/fuddlesworth/PlasmaZones/pull/1000)).
...
- **Restoring a maximized scrolling column no longer snaps out to full width first**: a restore click reached the check that guards a tiled window against being moved by other programs, and it ran before the strip had answered. The check read the smaller window as outside interference and pushed it back out to full width, so every restore bounced through full width on its way back. A window whose maximize state is in the middle of changing is now left alone, on X11 and XWayland windows as well as Wayland ones ([#1001](https://github.com/fuddlesworth/PlasmaZones/pull/1001)).
- **Maximizing a window on a scrolling monitor moves it once**: the maximize button put the window back where the strip had it and only then asked the strip what to do, so the window jumped twice before the animation ran, and the animation set off from wherever that second jump had left it. Your click now stands while the strip is asked, and the strip's answer drives a single animated move. A request the strip does nothing with puts the window back, so the button never stays lit for a state the strip did not agree to ([#1001](https://github.com/fuddlesworth/PlasmaZones/pull/1001)).
- **A maximize animates from where the window was**: the animation set off from the window's current position, which by then was already the size the compositor had maximized it to. Growing showed almost no movement and shrinking showed none at all. It now sets off from the position the window held before the maximize, on monocle tiling as well as scrolling columns ([#1001](https://github.com/fuddlesworth/PlasmaZones/pull/1001)).
- **Clicking maximize twice in quick succession does both presses**: the second click landed while the strip was still answering the first, and it was discarded, so a fast double-click maximized and stayed maximized. The second press is now remembered and applied once the first has settled ([#1001](https://github.com/fuddlesworth/PlasmaZones/pull/1001)).
- **Zone shader previews number their zones again**: the settings and editor shader previews draw the zone numbers into a picture and hand it to the renderer, and that picture never arrived, so the previews showed their zones bare. The numbers on the real overlay were never affected ([#998](https://github.com/fuddlesworth/PlasmaZones/pull/998)).
- **Decoration previews show what is behind the window again**: every pack that samples the scene behind a window drew as though there were nothing behind it. Blur, Duotone, Frosted Glass, Glass and the other glass packs showed a flat tinted slab instead of a blurred desktop, and Mosaic showed a tint with no cells at all. The wallpaper the preview stands in with never reached the shader. Zone and overlay shaders that sample the wallpaper were left without one in the same way ([#998](https://github.com/fuddlesworth/PlasmaZones/pull/998)).
- **A decoration pack looks the same in the browser as in its detail preview**: the browser card and the detail preview drew a pack on surfaces of different sizes, and a pack's blur runs in a buffer sized from the surface it is drawn on, so one pack came out as two different effects. Every decoration preview is drawn at one size now and scaled to fit where it is shown, which makes the card a smaller copy of the detail preview rather than a different picture ([#998](https://github.com/fuddlesworth/PlasmaZones/pull/998)).
...
- **Phosphor Glass groups its parameters**: the pack's thirteen parameters were one flat list and are now sorted into Blur, Shape, Content, Tint, Glow and Gradient sections in the parameter editor ([#984](https://github.com/fuddlesworth/PlasmaZones/pull/984)).
...
- **Pick a scrolling screen's template on the Overview page, including none at all**: a scrolling screen showed a strip preview and an explanation where the other two modes each offer a selector, so the template governing it could only be changed from the layout picker popup. It now has its own dropdown beside them, listing every template plus a None entry the other families do not need. Default follows the default template from Scrolling → Templates, as it does for the other two families, and None means this screen uses no template at all, so its columns keep the built-in width and height steps even while a default is set. None is a new state. Before it, a screen with no template of its own always inherited whatever default was set, with no way to opt out. The pick is applied with the rest of the page, so Discard reverts it ([#887](https://github.com/fuddlesworth/PlasmaZones/pull/887), [#929](https://github.com/fuddlesworth/PlasmaZones/pull/929)).
- **A scrolling screen says which template it uses**: the Overview page used to label a scrolling screen's preview with the bare mode name and tell you it uses no zone layout, which left the template that actually shapes the strip invisible. The preview now carries the template's name, and the note below it says what that template governs. A screen with no template of its own says so and points at the default template setting. When the template defines starting columns, the note also tracks how many are in use, so it is visible that a starting column is spent once a column has taken it and that later columns open at the template's ordinary width and display. The count is live and follows the strip. The two settings under Settings → Placement → Scrolling → Columns that a template overrides, Default width and Open new columns as, now say so on the row, because a screen with a template takes both from the template and the setting looked broken ([#887](https://github.com/fuddlesworth/PlasmaZones/pull/887)).
...
- **Resetting OSD and Popup decorations restores the built-in look**: resetting the Appearance → Decorations → Surfaces → OSDs or Popups page removed the built-in card border and shadow instead of restoring them. The built-in chrome is now a default layer underneath your edits, so reset brings it back, and setups that had already lost it heal on their own ([#798](https://github.com/fuddlesworth/PlasmaZones/pull/798)).
...
- **Window appearance and gaps are now rules**: window borders, title bars, corner radius, border colors, and gaps are controlled through the rule system instead of separate per-mode settings pages. A new General page under Appearance → Decorations edits the managed defaults, and per-window or per-context overrides are ordinary rules on the Rules page ([#699](https://github.com/fuddlesworth/PlasmaZones/pull/699)).
...
- **Choose which windows the appearance defaults apply to**: the Borders and Title bars cards on the Appearance → Decorations → General page have an "Apply to" option with three scopes, tiled and snapped windows, all normal windows, and all windows. New installs default to tiled and snapped windows, so the default border and hidden title bar stay off the desktop, panels, popups, and on-screen displays. Existing setups keep their current scope and can switch at any time ([#721](https://github.com/fuddlesworth/PlasmaZones/pull/721)).
...
- **Gaps use one shared model**: snapping zone padding and tiling inner gap are now the same setting. Set per-monitor gaps on the Appearance → Decorations → General page where a monitor should differ ([#699](https://github.com/fuddlesworth/PlasmaZones/pull/699)).
...
- **Master window balloons to 100% on notifications** ([#271](https://github.com/fuddlesworth/PlasmaZones/discussions/271)): whenever a Plasma notification popup appeared, KWin briefly reported a tiled window as minimized and restored it a millisecond or two later. PlasmaZones retiled without that window, so the master filled the screen for a moment and then shrank back, on every notification. Two changes fix it. Plasma's own shell surfaces, such as notifications, the emoji picker and KRunner, are no longer tracked for tiling. A minimize also has to last 75 ms before PlasmaZones acts on it, which a real minimize always does.
...
- **Build dependencies for packagers**: drop `layer-shell-qt` / `liblayershellqtinterface-dev`, add `qt6-wayland` / `qt6-wayland-dev`, add `wayland-scanner` (usually in `wayland-devel`), and add `vulkan-headers` and `vulkan-loader` (optional, for the Vulkan backend).
...
- **Enhanced label effects for branded shaders**: the zone numbers in the CachyOS, Fedora, Neon, and NixOS Drift shaders appeared as solid white. Each shader now draws its numbers in a style of its own, which is a digital shatter for CachyOS, frost crystals for Fedora, a flickering neon tube for Neon, and a hash grid for NixOS.
...
- **Pluggable algorithm architecture** with 11 tiling layouts: Master+Stack, Centered Master, BSP, Dwindle, Spiral, Columns, Rows, Grid, Wide, Three-Column, and Monocle
...
- Layout visibility filtering: control which layouts appear in zone selector per screen, virtual desktop, and activity
  ...
  - A layout with no restrictions set is visible everywhere
...
[1.15.10]: https://github.com/fuddlesworth/PlasmaZones/compare/v1.15.9...v1.15.11
...
[1.15.0]: https://github.com/fuddlesworth/PlasmaZones/compare/v1.14.1...v1.15.1
