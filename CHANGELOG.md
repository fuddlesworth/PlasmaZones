...
- **The daemon and the KWin effect must be installed together**: the tile-request signal grew fields for the scrolling strip's motion and then for windowed fullscreen, and a signal hook built against an old shape breaks rather than degrading, so the protocol version floor moved and the two reject a mismatched pairing at startup. Packagers must rebuild and ship both binaries from the same source ([#890](https://github.com/fuddlesworth/PlasmaZones/pull/890), [#895](https://github.com/fuddlesworth/PlasmaZones/pull/895)).
