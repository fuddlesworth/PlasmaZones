...
- **A window is no longer untiled because two of them share an application**: when PlasmaZones could not tell two windows of the same application apart, it dropped one from the batch and then treated it as no longer tiled, handing back its title bar and maximize state while the daemon still had it tiled ([#1031](https://github.com/fuddlesworth/PlasmaZones/pull/1031)).
