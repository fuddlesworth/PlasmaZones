...
- **The three set pages are all called Sets**: the Library pages under Animations, Decoration and Overlays were titled Motion Sets, Decoration Sets and Overlay Sets. Each now reads plain Sets, and the text on those pages and their search keywords say set rather than motion set, decoration set or overlay set. The section the page sits in already says which kind it is.
...
- **Exporting a layout no longer carries its overlay shader**: which shader a layout draws is now kept with your settings rather than inside the layout file, so an exported layout file describes zones and layout settings and nothing about shaders. Import it on another computer and it draws whatever your global default is there. To move assignments between computers, save an overlay set on the Sets page under Overlays. A set carries the global default and every per-layout override together. ([#1006](https://github.com/fuddlesworth/PlasmaZones/pull/1006))
