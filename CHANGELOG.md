...
- **Scrolling width and height presets now include three quarters and full size**: the default vocabulary the cycle-width and cycle-height shortcuts step through was one third, one half and two thirds. It now goes on to three quarters and the full work area, so a column can be walked up to full width without leaving the preset cycle. Your own list under Settings → Scrolling → Columns is left alone.
...
- **Gaps can go up to 200 pixels**: the inner and outer gap sliders under Settings → Windows stopped at 50, which on a large or high-resolution screen was too small a margin to be worth having. Every gap setting, the per-side outer gaps and the per-screen tiling overrides included, now accepts up to 200.
- **Window borders can be wider and rounder**: the corner radius under Settings → Windows stopped at 20 pixels and the border width at 10, which was not enough for the rounded look many themes use. The radius now goes to 50 and the width to 20, the same range the zone overlay corners already had, and window rules setting either value accept the same range.
