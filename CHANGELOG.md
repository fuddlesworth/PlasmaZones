...
- **An empty scroll strip says it is empty instead of drawing three windows that are not there**: a scrolling screen with nothing on the strip used to show a stand-in picture of three columns, with the outer two cut off at the edges to suggest the strip carried on past them. It was drawn by the same renderer that draws real windows, in the same fills, so it read as three real windows on a strip that held none, and it followed nothing you had configured. It is gone. The preview now shows an arrow along the direction the strip runs and says why it is empty, and the three reasons that used to share one picture are told apart: no windows on the strip yet, scrolling picked for the screen but not applied yet, and PlasmaZones not running. The card keeps its name and badge either way, so a screen still says which template governs it while nothing is on the strip. Every scrolling preview also gains a small mark on each edge the strip continues past, whether or not it currently holds windows, which is the one thing the old picture communicated worth keeping. The mark appears on the Monitors page, on the mode-change OSD, and on template cards in the layout picker, the template library and the template dropdowns. Where the preview is tied to a screen it follows that screen's strip direction, per-monitor overrides included. Where templates are browsed with no screen in mind it follows the global direction ([#971](https://github.com/fuddlesworth/PlasmaZones/pull/971)).
