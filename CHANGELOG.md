...
- **Shader sets for zone overlays**: your zone overlay shader assignments can be saved as a named set and applied again later, the same way decoration sets and motion sets already work. The page is Appearance → Overlays → Library → Overlay Sets. A set holds the global default and every per-layout override in one JSON file you can keep, share or import, and it covers shader assignments only, matching what the other shader pages deal with. Applying a set merges, so anything it does not cover keeps what it has. Saving, renaming and deleting a set take effect immediately, matching decoration sets, and only applying one is a pending change that Discard undoes. A set from another computer still works even though layouts are identified per computer: the global default and any layout you also have are applied, and the rest are skipped with a count so you know what was left out. A set naming a shader pack you do not have installed is refused instead, because none of it could be applied. ([#1006](https://github.com/fuddlesworth/PlasmaZones/pull/1006))

### Changed

- **Appearance → Overlays**: a new section beside Decorations and Animations, laid out the way Decorations is. Appearance and Assignments lead, then a Library holding your saved Overlay Sets and the shader pack browser. Appearance is the overlay's colours, label font, opacity, border and zone numbers; Assignments is which shader each layout draws. When the overlay appears while you drag stays a Snapping setting, on the Overlay page there. ([#1006](https://github.com/fuddlesworth/PlasmaZones/pull/1006))
- **Overlay shader assignment moved from the layout editor into the settings app**: which shader a zone overlay draws is a display preference rather than part of a layout, so the editor's shader dialog is gone and assignments live on the Assignments page above. It has one card for the global default and one per layout, each with the same shader picker and parameter editor the dialog had. Existing per-layout assignments are carried over automatically on the first start after updating. ([#1006](https://github.com/fuddlesworth/PlasmaZones/pull/1006))
