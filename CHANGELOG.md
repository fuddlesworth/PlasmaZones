...
- **A decoration stays on a window while it closes**: a decoration pack rode a closing window out of view only when PlasmaZones was playing the close animation itself. When anything else animated the window away, such as the slide a Plasma panel popup plays as you dismiss it, the pack stopped drawing the instant the window closed and the last of the animation played bare. The decoration now stays for as long as the compositor keeps drawing the window, so a decorated launcher or system tray popup wears its pack all the way out, and so does an ordinary window closing with the PlasmaZones animations turned off ([#950](https://github.com/fuddlesworth/PlasmaZones/pull/950)).
