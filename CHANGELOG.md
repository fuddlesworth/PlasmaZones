...
- **Layouts work as sizing templates on scrolling screens**: assigning a zone layout to a screen in Scrolling mode now teaches the strip that layout's proportions instead of doing nothing. The layout's column widths become the width presets the Cycle Column Width shortcut steps through on that screen, and a layout with stacked zones supplies the height presets too. Pick a template with the layout picker, the quick layout digits, or the layout cycle shortcuts, exactly as you would pick a layout on a snapping screen, and the choice follows the same per-screen, per-desktop, and per-activity assignment the other modes use. Editing the layout in the editor reflows columns that use a preset width right away. A rule action, Set scrolling template, applies a template per context, and clearing the template returns the screen to the preset lists from Settings → Scrolling → Columns. The picker's card and the switch OSD caption the choice as a column template, and the shortcut cheatsheet's layout rows explain their template meaning on a scrolling screen.
- **Rules can target a specific template**: on a scrolling screen the Active layout rule field now carries the template in use, the same way it carries the algorithm on a tiling screen, so a rule can set gaps, the overlay, or the lock for the screen showing a given template. The rule editor's Active layout picker offers a Template entry per layout. A rule matching the bare scrolling value now means a scrolling screen with no template; match the Mode field to address every scrolling screen.
