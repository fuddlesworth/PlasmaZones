...
- **Focus crosses monitors on a scrolling screen**: moving focus past the last column used to stop dead at the strip edge, even though moving a window there crossed onto the next monitor and the tiling mode's focus already crossed as well. The plain focus shortcuts now continue onto the neighbouring monitor, entering its strip at the window facing the edge you crossed, and the crossing also works when the neighbour runs Snapping or Tiling mode. For users who prefer niri's exact stop-at-the-edge behaviour, edge-stop variants of the column focus shortcuts ship unbound and can be given chords in the Shortcuts KCM, alongside unbound wrap variants that jump to the far end of the strip instead ([#897](https://github.com/fuddlesworth/PlasmaZones/pull/897)).
- **More scrolling verbs from niri**: Center Visible Columns (`Meta+Alt+Shift+C`) centers the group of fully visible columns as a block, the whole-span companion of Center Column. Focus Top Window and Focus Bottom Window (`Meta+Alt+V` and `Meta+Alt+Shift+V`) jump to the ends of the focused column. Switch Focus Between Floating and Tiled (`Meta+Alt+X`) moves focus between the floating windows and the tiled columns, returning to the most recent window on each side. Move to Floating and Move to Tiled ship unbound and give the float toggle a deterministic one-way form ([#897](https://github.com/fuddlesworth/PlasmaZones/pull/897)).
- **Absolute column width and window height over D-Bus**: four typed methods on org.plasmazones.Scrolling set the focused column's width or the focused window's height to an exact fraction or pixel value, the counterpart of niri's set-column-width and set-window-height for scripting. Shortcuts carry no value argument, so this surface is where the absolute forms live ([#897](https://github.com/fuddlesworth/PlasmaZones/pull/897)).
...
- **Two default shortcuts moved off Spectacle's chords**: Cycle Column Width moved from `Meta+Alt+R` to `Meta+Alt+D` with the reverse cycle on `Meta+Alt+Shift+D`, and Retile Windows moved from `Meta+Ctrl+R` to `Meta+Ctrl+T`. Spectacle registers every Meta-modified R chord for its screenshot and recording shortcuts, and the system hands a contested chord to whichever application registered first, so on a stock Plasma install these two either never fired or blocked Spectacle. Users who never rebound them pick the new chords up automatically ([#897](https://github.com/fuddlesworth/PlasmaZones/pull/897)).
- **The daemon and the KWin effect must be installed together**: the tile-request signal grew fields for the scrolling strip's motion, and a signal hook built against the old shape breaks rather than degrading, so the protocol version floor moved and the two reject a mismatched pairing at startup. Packagers must rebuild and ship both binaries from the same source ([#890](https://github.com/fuddlesworth/PlasmaZones/pull/890)).
...
- **Tab indicators stay behind the windows in front of them**: the indicator over a tabbed scrolling column used to draw on top of every window on the screen, so a floating window raised over the strip wore a stripe of the column behind it. The indicator now sits above the columns it belongs to and behind anything stacked over the strip, the way it would if it were an ordinary window ([#896](https://github.com/fuddlesworth/PlasmaZones/pull/896)).
