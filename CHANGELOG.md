...
- **Overlay sets**: your zone overlay shader assignments can be saved as a named set and applied again later, the same way decoration sets and motion sets already work. A set captures the global default and every per-layout override into one JSON file you can keep, share or import. Applying a set merges, so anything it does not cover keeps what it has. Saving, renaming and deleting a set take effect immediately, matching decoration sets, and only applying one is a pending change that Discard undoes. A set from another computer still works even though layouts are identified per computer: the global default and any layout you also have are applied, and the rest are skipped with a count so you know what was left out. A set naming a shader pack you do not have installed is refused instead, because none of it could be applied. ([#1006](https://github.com/fuddlesworth/PlasmaZones/pull/1006))

### Changed

- **Everything about the zone overlay is in one place**: which shader a zone overlay draws is a display preference rather than part of a layout, so the editor's shader dialog is gone. That assignment, along with the overlay's colours and fonts and the settings for when it appears while you drag, now lives under Appearance → Overlays beside Decorations and Animations. The Assignments page has one card for the global default and one per layout, each with the same shader picker and parameter editor the dialog had, and the Library page next to it browses and installs packs. Snapping keeps its own settings, including its on and off switch. Existing per-layout assignments are carried over automatically on the first start after updating. ([#1006](https://github.com/fuddlesworth/PlasmaZones/pull/1006))
