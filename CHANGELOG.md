...
- **More scrolling verbs from niri**: Center Visible Columns (`Meta+Alt+Shift+C`) centers the group of fully visible columns as a block, the whole-span companion of Center Column. Focus Top Window in Column and Focus Bottom Window in Column (`Meta+Alt+V` and `Meta+Alt+Shift+V`) jump to the ends of the focused column. Switch Focus Between Floating and Tiled (`Meta+Alt+X`) moves focus between the floating windows and the tiled columns, returning to the most recent window on each side. Move Window to Floating and Move Window to Tiled ship unbound, the same configuration-file binding as the focus variants above, and give the float toggle a deterministic one-way form ([#897](https://github.com/fuddlesworth/PlasmaZones/pull/897)).
...
- **The daemon and the KWin effect must be installed together**: the protocol between them widened several times this cycle. The snap restore call grew arguments for the cross-screen window reclaim, and the tile-request signal grew fields for the scrolling strip's motion and then for windowed fullscreen. A signal hook built against an old shape breaks rather than degrading, so the protocol version floor moved and the two reject a mismatched pairing at startup. Packagers must rebuild and ship both binaries from the same source ([#890](https://github.com/fuddlesworth/PlasmaZones/pull/890), [#892](https://github.com/fuddlesworth/PlasmaZones/pull/892), [#895](https://github.com/fuddlesworth/PlasmaZones/pull/895)).
...
- **Tab switches cross-fade**: switching tabs in a scrolling column now blends the tab you left into the one you picked, in place, over the same rectangle. Both tabs are ordinary windows at the same size and position, so the column reads as one surface changing its contents. The transition has its own entry, Tab Switched, under Settings → Animations → Motion → Scrolling, where the curve, the duration, and the shader can be set like any other. There is no default, so a tab switch stays an instant cut until you pick a pack. Six ship, and only these fit the event, because a tab shader reads the outgoing tab as well as the arriving one. Tab Fade blends the two in place. Tab Slide pushes one out of the column as the other follows it in. Tab Wipe sweeps the new tab across the old with an edge you can soften or sharpen. Tab Dissolve grains one into the other. Tab Flip turns the column over like a card. Tab Zoom pulls the old tab toward you as the new one rises from behind it ([#908](https://github.com/fuddlesworth/PlasmaZones/pull/908)).
- **Three tab switches in the Phosphor style**: the branded set gains the one animation class it did not cover. Phosphor Swap runs a lit boundary across the column, draining the tab you left to a dark navy silhouette ahead of it and bringing the one you picked back up to full colour behind it, with ember sparks feeding the boundary and a rose afterglow in its wake. Phosphor Transfer breaks the old tab into luminous dust along a climbing frontier and re-forms the new one out of it, so the same material reads as reshaped rather than replaced. Phosphor Iris opens a turbulent ring of plasma from the centre of the column, already carrying the tab you picked inside it. All three take the brand gradient from cyan through to rose and expose it as colour settings like the rest of the set ([#908](https://github.com/fuddlesworth/PlasmaZones/pull/908)).
...
- **A newly activated tab no longer flies up from below the screen**: the tabs of a scrolling column that are not showing are held off the bottom of the desktop so they cannot take clicks meant for the visible one. Activating a tab moved it from there back into the column, and nothing said that this was a swap rather than a journey, so the arriving tab travelled the full height of the screen while the tab it replaced vanished on the spot. The two now trade places without moving, which is what the switch actually is ([#908](https://github.com/fuddlesworth/PlasmaZones/pull/908)).
