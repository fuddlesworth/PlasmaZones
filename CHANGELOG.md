...
- **Drop indicator for the scroll strip**: while a drag re-insert is in progress, the exact space the window will land in is highlighted, so the drop is no longer a guess. It has its own section under Settings → Scrolling with a switch, a fill color, a fill opacity, a border color, a border width, and a corner radius. The two colors follow your color scheme until you pick one, and the defaults match the snapping zone overlay so the two highlights look like the same thing ([#873](https://github.com/fuddlesworth/PlasmaZones/pull/873)).
