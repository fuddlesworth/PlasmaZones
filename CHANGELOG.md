...
- **Meta+wheel scrolls through columns**: on a screen in Scrolling mode, holding Meta and scrolling the mouse wheel focuses the next column and slides the strip, matching niri's Mod+wheel navigation. Horizontal wheel ticks and two-finger horizontal touchpad scrolls work too. The binding is only active while at least one screen uses Scrolling mode ([#852](https://github.com/fuddlesworth/PlasmaZones/pull/852)).
...
- **Scrolling shortcut defaults work on every keyboard layout**: four defaults sat on punctuation keys that many non-US layouts put behind Shift, where a global shortcut can never fire on Wayland. Consume Window moved from Meta+Alt+; to Meta+Alt+I and Expel Window from Meta+Alt+' to Meta+Alt+Shift+I. Increase Column Width moved from Meta+Alt+= to Meta+Alt+W and Decrease Column Width from Meta+Alt+- to Meta+Alt+Shift+W. A binding you customized yourself is untouched.
- **The reverse preset cycles have default shortcuts**: the scrolling letter shortcuts now follow one rule, where Shift on the same letter performs the opposed action. Cycle Window Height moved from Meta+Alt+Shift+R to Meta+Alt+H to free that slot, Cycle Column Width Back is Meta+Alt+Shift+R, and Cycle Window Height Back is Meta+Alt+Shift+H. Both reverse cycles previously shipped unbound.
- **"Expand Column to Available Width" is now "Grow Column into Empty Space"**: the old name was hard to tell apart from Maximize Column. The action claims the empty space visible on screen for the focused column and leaves every other column alone.
- **The shortcut cheatsheet explains the Scrolling actions**: hovering a row in the Scrolling group shows a plain-language description of what the action does, so consume, expel and the column sizing verbs no longer require guessing.
- **Saved settings no longer freeze the defaults**: the configuration file now stores only values that differ from their default, and saving prunes previously stamped defaults. Before this, any save wrote every setting to disk, so a later release that retuned a default never reached a configuration saved under the old one. Settings you actually changed are kept exactly as before.
