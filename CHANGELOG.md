...
- **"Deactivate while held" trigger for always-active drag overlay** ([#249](https://github.com/fuddlesworth/PlasmaZones/issues/249)): when "Activate on every drag" is on, holding a configurable modifier or mouse button suppresses the zone overlay for the rest of the drag. Configured under Snapping → Behavior → Triggers; the row only appears when "Activate on every drag" is enabled. Default is empty — Esc continues to cancel the drag entirely as before. Users coming from FancyZones can bind Right Mouse Button to match the familiar pattern.
