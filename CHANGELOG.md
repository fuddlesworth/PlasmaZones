...
- **A rule no longer offers animation events it cannot change**: the event list on the Override animation actions held around sixty events, and only ten of them are ones a rule can affect. A rule matches windows, so it can only change an animation that belongs to a window. Picking a virtual desktop switch, a scrolling strip movement, an OSD, a side panel, an editor widget or a Plasma shell popup gave you a rule that saved without complaint and then did nothing, with nothing anywhere to say why. Those events are no longer listed. A rule you already made that names one keeps it, still shown by name, now with a note beside it saying the event is not driven per window and pointing you at the Animations pages, where setting it works ([#952](https://github.com/fuddlesworth/PlasmaZones/pull/952)).
