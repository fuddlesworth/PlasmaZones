...
- **Rectangle-driven packs assigned to Window move or Window resize swept in from the screen corner**: packs that derive their motion from the window rectangle (Window morph, Fold, Stretch, Flow) read a zero start rectangle during a drag, so the first fraction of a second of every grab drew the window flying in from the top-left corner of the screen, or not at all. The drag now anchors its start rectangle at the grab ([#755](https://github.com/fuddlesworth/PlasmaZones/pull/755)).
