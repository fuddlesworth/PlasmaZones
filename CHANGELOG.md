...
- **More scrolling verbs from niri**: Center Visible Columns (`Meta+Alt+Shift+C`) centers the group of fully visible columns as a block, the whole-span companion of Center Column. Focus Top Window in Column and Focus Bottom Window in Column (`Meta+Alt+V` and `Meta+Alt+Shift+V`) jump to the ends of the focused column. Switch Focus Between Floating and Placed Windows (`Meta+Alt+X`) moves focus between the floating windows and the tiled columns, and it has since grown to cover every mode, described in its own entry above. Move Window to Floating and Move Window to Tiled ship unbound, the same configuration-file binding as the focus variants above, and give the float toggle a deterministic one-way form ([#897](https://github.com/fuddlesworth/PlasmaZones/pull/897)).
