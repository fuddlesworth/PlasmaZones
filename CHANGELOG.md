...
- **A window floats back to its own remembered position, not another window's**: PlasmaZones remembers where a window was floating so it can be put back there. When a window had nothing remembered, it fell back to a position remembered for a different window of the same application, on the grounds that a new window may as well open where the last one of that app was. An application only keeps its last sixteen windows on record, so a window that had been open a while could find that record full of windows that had since closed, borrow one of their positions, and be moved to whatever monitor that closed window had been on. Snapping, tiling and scrolling each resolved this differently, and two of them had already stopped falling back. All three now put a window back only where that same window was, and a window with nothing remembered stays where it is ([#1030](https://github.com/fuddlesworth/PlasmaZones/pull/1030)).
