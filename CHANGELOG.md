...
- **Global "auto-assign new windows for all layouts" toggle** ([#370](https://github.com/fuddlesworth/PlasmaZones/issues/370)): a new master switch in Snapping Behavior → Window Handling that, when on, makes every manual layout auto-assign new windows to its first empty zone — regardless of each layout's individual auto-assign icon. Effective behavior is `globalToggle OR layout.autoAssign`, so the per-layout icon still works normally on its own when the global is off. Default is off, preserving existing per-layout-only behavior; users who didn't realize auto-assign was a per-layout setting (the discoverability problem reported in #370) can now flip one switch instead of toggling each layout. While the global is on, the per-layout corner icon, the per-layout context menu entry, the layout-name "Auto/Manual" badge (in the Layouts grid, the layout combo box, the zone selector, the layout picker, and the layout-switch OSD) all show the effective on state and the per-layout toggle is disabled with a tooltip that points back at the global setting, so the displayed state always matches actual snap behavior.
