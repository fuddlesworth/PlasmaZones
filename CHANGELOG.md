...
- **Minimizing a window now plays its animation**: the Minimized animation event used to fire only when a window was restored, so the way down was always an instant blink. Any animation assigned to the event now plays in both directions, in reverse while minimizing and forward while restoring ([#776](https://github.com/fuddlesworth/PlasmaZones/pull/776)).
- **Genie and Phosphor Siphon, two minimize-to-icon animations**: Genie pours the window into its taskbar icon like the classic magic lamp, with the edge nearest the icon leading, and restoring pours it back out. Phosphor Siphon does the same journey as separated luminous streams that carry the brand gradient and shed ember sparks, each stream fading into the icon as it lands. A window that is not in any task manager drains into its own bottom edge instead ([#776](https://github.com/fuddlesworth/PlasmaZones/pull/776)).
