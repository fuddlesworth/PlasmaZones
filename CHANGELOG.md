...
- **Every blur-family decoration has brightness, contrast, saturation, vibrancy and grain controls**: the blur packs only offered a tint before, so a blurred pane could not be lifted, muted or dithered the way a stock blur-behind effect allows. Blur, glass, frosted glass, rippled glass, rain glass and phosphor glass now expose the set, each defaulting to the look it had, and duotone gains the grain. Vibrancy is the Hyprland control: it boosts saturation only where a colour is already saturated and bright, so a muted wallpaper stays muted. The grain is a fine noise over the blur that hides banding in smooth gradients, and it is the same on every GPU.
...
- **The blur-family packs blur with a dual Kawase pyramid**: blur, glass, frosted glass, rippled glass, rain glass, duotone and phosphor glass used a two-pass Gaussian at quarter resolution, which was smooth but could not reach far. They now run a four-level pyramid down to a thirty-second of the canvas and back, the same shape stock blur effects use, so a radius up to 256 px stays smooth at the same cost as before. A small radius uses a shallow pyramid, so the look at the old default values is unchanged. Under the hood a surface pack may now declare up to eight buffer passes, each at its own resolution through a new `bufferScales` list, and the daemon and the KWin effect both honour it.
