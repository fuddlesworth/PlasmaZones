...
- **Fixed-size X11 windows no longer jump to the top of the screen while scrolling off**: a scrolling column window that cannot fill its column, like a game launcher with a fixed size, is committed centered in the column. The paint that keeps a column visible while it scrolls out of view ignored that centering and drew the window at the column's top left corner, so it leapt to the top of the screen the moment it left the viewport and rode there until the scroll settled. It now leaves and arrives at the centered position it actually occupies. Wayland windows were never affected because only X11 frames are size-constrained up front.
...
[Unreleased]: https://github.com/fuddlesworth/PlasmaZones/compare/v3.3.5...HEAD
[3.3.5]: https://github.com/fuddlesworth/PlasmaZones/compare/v3.3.4...v3.3.5
