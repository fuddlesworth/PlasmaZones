...
- **A border pack rounded a corner you had squared, and erased the pane's own square corner doing it**: a corner radius of 0 means square, and the Borders family drew it as an arc the width of its own band, up to 20 px at the widest setting. Because a border pack clears the window content outside its band, that arc also cut the matching corner off whatever backdrop pack sat under it, so a squared pane came out rounded and clipped. A radius of 0 now stays square at both ends, and the band mitres the corner the way a drawing program would. A radius above 0 is unchanged, where the band still runs from the radius you set out to that radius plus its width. ([#1126](https://github.com/fuddlesworth/PlasmaZones/pull/1126))
- **A glow or drop shadow still curved around a corner the pane had squared**: the two packs traced the pane's own outline correctly but held their halo back using a single rounded corner for both ends, so squaring the bottom corners left the halo reaching past them. Both ends now follow the same shape the outline does. ([#1126](https://github.com/fuddlesworth/PlasmaZones/pull/1126))
