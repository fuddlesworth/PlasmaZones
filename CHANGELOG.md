...
- **Scoped exclusion rule actions**: two new rule actions split the blanket Exclude. "Exclude from tiling, snapping, and scrolling" keeps an application's windows out of every placement engine while its borders, decoration packs, and animations still apply, which suits tools like deskflow that should stay floating but keep their styling. "Exclude from borders and decoration packs" turns off only PlasmaZones' own window styling and leaves placement and animations alone. The existing Exclude action keeps its full meaning, and the "Exclude an app from tiling, snapping, and scrolling" quick-start template now uses the placement-only action, with a new "Undecorate an app" template beside it. Rules already created from the old template keep their stored blanket behavior ([#880](https://github.com/fuddlesworth/PlasmaZones/pull/880)).
