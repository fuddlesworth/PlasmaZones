...
- **Closed windows reopen the way they left, and a logout brings the strip back intact**: a window floated in Scrolling mode used to re-tile when closed and reopened, because every fresh window writes a bookkeeping note into the placement memory before the restore runs, and the restore treated that note as the window's final answer, so it never looked for the state the window actually closed in. Reopening also used to bring back the oldest remembered placement for an application rather than the newest, so long-gone arrangements could outrank the state a window actually closed in. And logging out recorded each window's column only after earlier closes had already shifted the numbering, so the next login rebuilt a collapsed strip with windows parked past the screen edge. A reopen now restores the newest remembered state that does not belong to a still-open window, closes record their placement while the engine still knows the window, and a burst of closes remembers the columns as they stood before the burst began. The same repairs apply in Tiling mode ([#886](https://github.com/fuddlesworth/PlasmaZones/pull/886)).
