...
- **Every pack that draws a pane's outline can round or square its bottom corners, and its edge feather is adjustable**: "Round bottom corners" is on by default, and turning it off rounds only the top two, which suits a pane sitting against a panel or the screen edge. "Edge softness" sets the anti-alias feather of the edge. The eight Blur-category packs gained both controls, and so did the ten Borders packs, Glow and Drop Shadow, so every pack in a chain can describe the same shape. ([#1123](https://github.com/fuddlesworth/PlasmaZones/pull/1123), [#1126](https://github.com/fuddlesworth/PlasmaZones/pull/1126))
