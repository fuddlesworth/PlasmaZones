...
- **Tab switches cross-fade**: switching tabs in a scrolling column now blends the tab you left into the one you picked, in place, over the same rectangle. Both tabs are ordinary windows at the same size and position, so the column reads as one surface changing its contents. The transition has its own entry, Tab Switched, under Settings → Animations → Motion → Scrolling, where the curve, the duration, and the shader can be set like any other. Tab Fade ships as the default and is the only pack that fits the event so far, since a tab shader reads the outgoing tab as well as the arriving one ([#908](https://github.com/fuddlesworth/PlasmaZones/pull/908)).
