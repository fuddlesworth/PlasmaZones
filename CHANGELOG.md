...
- **Creating an algorithm from a template keeps its behavior intact**: the New Algorithm wizard used to rewrite a template's metadata from its four checkboxes, which silently dropped settings the template's code relies on, such as the master zone marker, the template's own defaults, and the flags behind resize reflow and the focus-following spotlight. Only the name and id are personalized now, and everything else travels with the copy ([#779](https://github.com/fuddlesworth/PlasmaZones/pull/779)).
