...
- **Layouts that opt out of minimum window sizes are now left alone**: Tatami, Floating Center, Cluster, and Theater each declare that minimum sizes are not something they work in, but the correction pass ran over their zones anyway and reshaped them. Zones from a layout that opts out are now used as the layout produced them. If a window in one of these four had been nudged to meet its minimum size, it keeps the size the layout gives it instead ([#779](https://github.com/fuddlesworth/PlasmaZones/pull/779)).
