...
- **Moving a window to the other monitor with the keyboard could throw it back**: with a window snapped on one monitor, Meta+Alt+Shift+Left or Right into a zone on the other monitor could send it back to where it started, and holding the keys down could leave several windows bouncing between the monitors and swapping places after the keys were released. PlasmaZones kept the window's old zone on the monitor it left alongside the new one and treated the pair like a window shown on two desktops, so the next update put it back in the old zone. A window moved to another monitor now leaves its old placement behind. ([#1124](https://github.com/fuddlesworth/PlasmaZones/discussions/1124))
- **A window moved between a tiling monitor and a snapping one could land back where it started**: moving a tiled window onto a snapping monitor with the keyboard sometimes put it back in its old tile a moment later, depending on how quickly the application answered the resize. A window that moved to another monitor while it was tiled was also pulled back into its old zone when the monitor it had left went back to snapping. Both now stay on the monitor they were moved to.
- **Snapping a window by number after moving it to another monitor put it back on the first one**: after the focused window was moved to another monitor with a KWin shortcut, a snap-to-zone shortcut snapped it into a zone of the monitor it had just left. PlasmaZones now follows the focused window when it changes monitor.
- **Floating a window snapped with the keyboard did not bring it back to where it was**: a window snapped with a keyboard shortcut or through the D-Bus interface forgot the position and size it had before, so floating it again left it at the size of the zone. It now returns to where it was before the snap, and a move to a desktop whose layout has no matching slot puts it back there too.
- **With auto-assign on, a window moved to another desktop could land in the wrong zone**: sending a snapped window to another desktop running the same layout put it in the next empty zone instead of the one it was in, because the window's own place on the desktop it came from counted as taken. It now keeps its zone, the same as with auto-assign off, and a desktop running a different layout gives it the matching slot there.
- **`Control.snapWindowToZone` ignored the screen it was given**: the D-Bus call snapped the window on whichever screen first had a matching zone, so with the same layout on two monitors it could land on the wrong one. It now snaps on the screen named in the call.
