...
- **A fullscreen game joined its launcher's column on a scrolling screen**: a game that opens fullscreen was placed on the strip like any other window, and because it shares an app name with its launcher it usually landed as a tab in the launcher's column. The tab bar could show up over the game, and the first time you used a scrolling shortcut the game was pulled out of fullscreen and stretched across the strip, taking the launcher with it. It happened on some launches and not others, which made it look random. A window that is already fullscreen when it opens is now left alone, the same way snapping and tiling screens treat it, and it joins the strip only if it later leaves fullscreen. Seen with World of Warcraft started from Battle.net under Proton. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **The tab bar could draw over a fullscreen window on a scrolling screen**: a tiled window that went fullscreen by itself, such as a game, a video or a browser after F11, could end up with its column's tab bar painted on top of it. PlasmaZones stops treating a window as part of the strip while it is fullscreen, but the next layout update quietly undid that, so it depended on whether anything else on the strip changed in the meantime. A window in its own fullscreen now stays out of the strip until it leaves fullscreen, so the tab bar is drawn underneath it and the window no longer slides when the strip scrolls behind it. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **Scrolling shortcuts pulled a window out of fullscreen**: moving focus along the strip, by shortcut or by the wheel, took any fullscreen window on that screen out of fullscreen first. A game that only runs at one size then fought the column it was given and could leave its whole column stretched across the screen. Moving around the strip now leaves a fullscreen window alone. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **A fullscreen window kept its place on the strip**: a tiled window that went fullscreen by itself still held its column, so the windows beside it did not close up, focus could land on the hidden column, and a game could sit as a tab inside its launcher's column. The window now leaves the strip for as long as it is fullscreen and returns to the same column, width and tab group when it leaves fullscreen. PlasmaZones' own windowed fullscreen is not affected. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **Windowed fullscreen turned off every decoration on the monitor**: with "Stop decorating while a window is fullscreen" turned on, putting a column into windowed fullscreen removed the decorations from the panels and from everything else on that monitor until you toggled it back. Windowed fullscreen fills the work area and leaves the panels on screen, so it no longer counts as fullscreen for that setting. The window itself still has no border while it is held. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
- **Maximizing one tab maximized the whole tab group**: clicking the maximize button on a tabbed window, or an app maximizing itself, stretched the entire column, and the column stayed stretched after that window was gone. A game that opened as a tab beside its launcher could leave the launcher filling the screen. The window that asked is now moved into its own column and maximized alone. The maximize shortcut still maximizes the focused column with all of its tabs. ([#1116](https://github.com/fuddlesworth/PlasmaZones/pull/1116))
...
- **Focus Window Top and Focus Window Bottom follow the same rules as the new tab shortcuts**: they now say so and do nothing when a floating window has focus, instead of quietly moving the placed window that was focused before it. After you have scrolled the strip away from the focused column, they also bring the window they focus back into view rather than leaving it off-screen. Focusing up or down within a column with the ordinary direction keys does the same. ([#1071](https://github.com/fuddlesworth/PlasmaZones/pull/1071))
...
- **The pointer no longer vanishes after a strip shader pass on a scrolling screen**: with a strip pack assigned under Settings → Animations → Motion → Scrolling, the strip pass hides the compositor cursor while it paints the strip and draws its own copy, and it only shows the cursor again from within the effect's paint hooks. The effect could leave the paint chain the instant the settle fade finished, one frame before the hook that shows the cursor ran, so the pointer stayed invisible until a later scroll happened to run it. Decorations or tab indicators kept the effect in the chain permanently, which is why only some setups saw it. The effect now stays in the chain while the cursor is hidden, so the pointer always comes back. ([#1062](https://github.com/fuddlesworth/PlasmaZones/pull/1062))
