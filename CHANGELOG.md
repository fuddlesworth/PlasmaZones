...
- **Strip shaders no longer smear the wallpaper**: a strip pack assigned under Settings → Animations → Scrolling ran over a capture of the whole screen, so the desktop background and anything kept below the columns moved and blurred along with them, which made the motion look wrong since only the columns are scrolling. The pack now only ever displaces the columns themselves. The wallpaper and keep-below windows stay exactly where they are underneath, the same way notifications and panels were already kept sharp on top, and a translucent or blurred window keeps its blur and its see-through look for the whole scroll. Strip Chromatic and Phosphor Gate were adjusted for the gaps between columns, so a colour fringe or an edge glow stays on the window it belongs to instead of spilling onto the desktop ([#1064](https://github.com/fuddlesworth/PlasmaZones/pull/1064)).
