...
Proposed — design only. No code has been written. This document covers the full
scope across all tiers so the architecture can be reviewed before implementation.
...
- The validator bakes every animation pack **with and without** `PLASMAZONES_KWIN`
  and is wired into `test_animation_shader_bake.cpp`'s CI coverage.
...
- `plasmazones-shader-validate <pack>` reports per-stage OK/errors over both
  `data/shaders/*` and `data/animations/*`, runs in CI over all bundled packs, and
