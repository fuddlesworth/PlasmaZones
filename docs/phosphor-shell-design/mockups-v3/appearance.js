...
window.PhosphorAppearance = (() => {
  ...
  function create({root,desktop,icon,getSettings,setSettings,presets,onClose,notify}) {
    ...
    function scene(wall,mini=false) {
      return `<div data-fit="${mini?'fill':data.displays[data.display].fit}" class="ap-scene ${mini?'ap-scene-mini':''}">${artwork(wall)}<div class="ap-scene-bar"><span class="phosphor-mark" aria-hidden="true"></span><i></i><span class="ap-tiny-map"><b></b><b></b><b></b></span><span>10:24</span></div><div class="ap-scene-panes"><div><span></span><i></i><i></i><i></i></div><div><span></span><b></b></div></div>${mini?'':`<div class="ap-scene-caption"><span>DESKTOP PREVIEW</span><b>${esc(wall.name)}</b><small>${wall.description||'Your own view.'}</small></div>${button('peek',`${icon('grid')} View on desktop`,'class="ap-preview-link"')}`}</div>`;
    }
