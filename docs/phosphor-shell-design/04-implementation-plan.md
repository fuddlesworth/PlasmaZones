...
### 2.1: `phosphor-service-pipewire` post-mortem *(shipped on feat/phase-2.1-pipewire-service)*
...
9. **Tests: smoke + role pinning (S, ~1 day).** No real PipeWire on CI; the lifecycle path is exercised inline in `tests/test_smoke.cpp` (construction defaults under no daemon, `disconnect()` before `connect()` no-op, no spurious signal emissions during loop teardown). A dedicated PipeWire-loop fixture lands only if a later milestone needs integration-style coverage; the smoke harness is sufficient for the public contract today.
