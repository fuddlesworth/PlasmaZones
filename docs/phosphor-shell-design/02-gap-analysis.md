...
| 2 | **(shipped, PR #535) PopoutService** | One service owns lifetime / focus / screen affinity / exclusive-zone arbitration / animations for popouts | DMS `Services/PopoutService.qml` (25 KB)                                        | `phosphor-popout`: `IPopoutService`, `PopoutController`, `PopoutHost.qml`. Replaces today's `PanelPopupHost`.    | M      | ~~**P0**~~   |
