...
| 2 | **PopoutService**                    | One service owns lifetime / focus / screen affinity / exclusive-zone arbitration / animations for popouts | DMS `Services/PopoutService.qml` (25 KB)                                        | `phosphor-popout`: `IPopoutService`, `PopoutController`, `PopoutHost.qml`. Replaces today's `PanelPopupHost`.    | M      | **P0**   |
...
| Gap                                | Reference                                       | Notes                                                                                              | Effort | Priority |
|------------------------------------|-------------------------------------------------|----------------------------------------------------------------------------------------------------|--------|----------|
| Plugin manifest + versioned ABI    | DMS `plugin-schema.json`, Noctalia `manifest.json` | C++/QML hybrid: compiled C++ plugins via a Phosphor-native loader (Qt's `QPluginLoader` with our own metadata schema); QML-only plugins via signed bundles. | L | **P1** |
