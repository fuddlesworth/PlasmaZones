...
5. **Typed `IpcHandler` + `phosphorctl` CLI.** Keep our existing D-Bus adaptors; add a typed IPC façade in QML + a single statically-linked CLI binary, so the Phosphor compositor (and, optionally, foreign compositors in guest-shell mode) bind keys to `phosphorctl call launcher.toggle` instead of raw D-Bus introspection.
6. **Connected-corner bar geometry.** DMS's `BarCanvas.qml` shape language, popouts grow out of the bar as one continuous painted surface, is the single most distinctive visual idea across the four shells, and the easiest to differentiate Phosphor on. We already have the shader stack; the geometry is QML + JS.
