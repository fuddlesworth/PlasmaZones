# phosphor-windowrule Table-Driven Refactor — Implementation Plan

Branch: `refactor/windowrule-table-driven`
Base: `v3.1`
...
**File:** `libs/phosphor-windowrule/include/PhosphorWindowRule/MatchTypes.h`
...
**File:** `libs/phosphor-windowrule/include/PhosphorWindowRule/WindowQuery.h`
...
**File:** `libs/phosphor-windowrule/tests/test_matchtypes.cpp`
...
**File:** `libs/phosphor-windowrule/include/PhosphorWindowRule/RuleAction.h`
...
**File:** `libs/phosphor-windowrule/src/ruleaction.cpp`
...
**File:** `libs/phosphor-windowrule/src/ruleaction.cpp` — in `registerBuiltins()`
...
**File:** `libs/phosphor-windowrule/tests/test_actionregistry.cpp`
...
## Phase 3 — Settings Layer Consolidation (`windowruleauthoring.cpp`)
...
PickerCategory actionCategory(const QString& type)
{
    ...
    auto desc = PhosphorWindowRule::ActionRegistry::instance().descriptor(type);
...
With:
```cpp
if (PhosphorWindowRule::ActionRegistry::instance().hasTag(action.type, PhosphorWindowRule::ActionType::Tag::Effect)) {
```

### 4.2  Settings: `windowrulemodel.cpp`
...
6. `windowruleauthoring.cpp` (GPL) — `fieldDescription()` + `fieldCategory()` **(~4 lines)**
...
3. `windowruleauthoring.cpp` — one `ActionLabels` entry **(~5 lines)**
