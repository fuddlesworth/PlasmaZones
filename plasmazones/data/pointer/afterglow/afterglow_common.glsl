...
// under 0.5 / (1 - persistence) LSB: 2.78 LSB at 0.82, so the highest
// stalled level is 2/255. That residue never clears on its own while the
...
// level is under the floor: 0.82 stalls at 2/255 = 0.0078 against a floor
...
// trailSeconds of samples), at the lowest refresh rate the family designs
// for: at 20 Hz that is 20 frames, and 0.82^20 = 0.019 is under the floor
// where 0.87^20 = 0.062 and 0.9^20 = 0.12 were not and would have cut the
// stroke off at the rect's edge there.
const float kCoverageFloor = 0.02;
const float kMaxPersistence = 0.82;
