                                               ...
                                               bool animateMinimized, bool* outOwnsResolvedLeg)
{
    ...
    // Kept because its fallback is safe, but it is NOT why the invariant holds.
    const auto resolved =
    ...
    // (window.move during placeIn, window.focus interrupting placeOut)
    // leave a stale timer that tears down the SUCCESSOR
