...
bool PlasmaZonesEffect::isEligibleForTilingNotify(KWin::EffectWindow* w) const
{
    ...
    // are never eligible for tiling notification. KWin's InternalWindow::minSize()
    // segfaults when the backing QWindow is null. See discussion #511.
    if (w && w->window() && w->window()->isInternal()) {
        qCDebug(lcEffect) << "isEligibleForTilingNotify: rejected (internal window)" << getWindowId(w);
        return false;
    }
