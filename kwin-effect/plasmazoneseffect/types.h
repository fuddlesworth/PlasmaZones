...
struct ShaderTransition
{
    ...
    /// surface quad; > 0 means it subdivides that quad into an NxN grid
    /// (via `WindowQuadList::makeRegularGrid`) so a custom vertex shader
    /// has interior vertices to displace — the `flow` window-move effect
    /// lags trailing grid rows behind the leading edge. Only honoured
    /// when `surfaceExtent` is also true.
