 ...
 * the actions (Apply, hidden while active, then the same inline icon-button
 * strip as a RuleRow: edit, export, delete).
 *
...
ExpandableRowDelegate {
    ...
    ColumnLayout {
        ...
        // Secondary line at the SAME recipe as a RuleRow's match summary and a
        // pack row's description teaser (default size, 0.7 opacity) — not
        // disabledTextColor, which reads as inactive rather than secondary.
        Label {
            Layout.fillWidth: true
    ...
    // regardless of how long the description is. Default-size text at the
    // RuleRow secondary-text opacity (not smallFont-on-disabledTextColor,
    // which double-dimmed the row's one piece of freshness information into
    // near-illegibility). The store always carries `modified` (the set
    // file's mtime), but a set file whose mtime the filesystem cannot report
    // would yield an invalid date, so render only a valid one rather than
    ...
    // Inline action strip — the SAME trailing pattern as a RuleRow's
    // edit / duplicate / delete cluster: every action visible at one glance
    // and one click, no overflow menu to open first.
    ToolButton {
        icon.name: "document-edit"
        Layout.alignment: Qt.AlignVCenter
        ToolTip.text: i18n("Edit set")
        ToolTip.visible: hovered
        Accessible.name: i18n("Edit set %1", row.setName)
        onClicked: row._openDialog(editDialogLoader, function (dlg) {
    ...
    ToolButton {
        ...
        Layout.alignment: Qt.AlignVCenter
        ToolTip.text: i18n("Export set")
        ToolTip.visible: hovered
        Accessible.name: i18n("Export set %1", row.setName)
        onClicked: row._openDialog(exportDialogLoader, null)
    }
    ...
    ToolButton {
        ...
        ToolTip.visible: hovered
        Accessible.name: i18n("Delete set %1", row.setName)
        onClicked: row._openDialog(deleteConfirmLoader, null)
    }

    // ── Dialogs ─────────────────────────────────────────────────────────

    // The four dialogs are built on FIRST USE, not up front. The Repeater
    // rebuilds every row whenever setsChanged fires (which the store also
    // raises for live-state edits), and a row that has never been interacted
    // with has no reason to carry four popups through that.
    ...
    // NOT header content, despite living under the shell's default alias like
    // everything above: a Loader is an Item, so bare in the header RowLayout
    // each one would claim a spacing slot even at zero width — four phantom
    // slots of dead space trailing the delete button. Layouts skip invisible
    // items entirely, and the invisible parent is harmless to the popups: a
    // Kirigami dialog renders in the window overlay and the export FileDialog
    // is a native window, so neither ever draws under this Item.
    Item {
        visible: false
