...
QJsonObject AutotileConfig::toJson() const
{
    ...
    json[InsertPositionKey] = insertPositionToString(insertPosition);
...
AutotileConfig AutotileConfig::fromJson(const QJsonObject &json)
{
    ...
    if (json.contains(InsertPositionKey)) {
        config.insertPosition = stringToInsertPosition(json[InsertPositionKey].toString());
    }
