...
void Daemon::initEnginesAndWiring()
{
    ...
    // relay entry points. Its stake is zone occupancy, queried across every
    // store, so a window that left a desktop has to stop being listed as an
    // occupant of the zone it was snapped into there.
    ...
    wireStickyMembershipUpdates(this, m_windowTrackingAdaptor, m_scrollEngine.get(), m_windowRegistry.get());
