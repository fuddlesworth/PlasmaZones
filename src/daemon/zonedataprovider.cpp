...
void ZoneDataProvider::setZones(const QVariantList& zones)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        qDebug() << "ZoneDataProvider: zone" << i << "pixels:" << px << py << pw << ph << "fillR:" << fillR
                 << "fillA:" << fillA << "borderA:" << borderA;
        ...
        setPixel(img, i, ROW_FILL, z.value(QLatin1String("fillR")).toFloat(), z.value(QLatin1String("fillG")).toFloat(),
                 z.value(QLatin1String("fillB")).toFloat(), z.value(QLatin1String("fillA")).toFloat());

        // Row 3: border color (RGBA 0-1)
        setPixel(img, i, ROW_BORDER, z.value(QLatin1String("borderR")).toFloat(),
                 z.value(QLatin1String("borderG")).toFloat(), z.value(QLatin1String("borderB")).toFloat(),
        ...
        setPixel(img, i, ROW_PARAMS, z.value(QLatin1String("shaderBorderRadius"), QVariant(8.0)).toFloat() / 255.f,
                 z.value(QLatin1String("shaderBorderWidth"), QVariant(2.0)).toFloat() / 255.f, isH,
