...
void WindowTrackingAdaptor::windowActivated(const QString& windowId, const QString& screenName)
{
    ...
    if (!zoneId.isEmpty() && m_settings && m_settings->moveNewWindowsToLastZone()) {
        ...
        if (!m_autoSnappedWindows.contains(windowId)) {
            ...
            qCDebug(lcDbusWindow) << "Updated last-used zone to" << zoneId << "from focus on" << windowClass << "screen"
                                  << screenName;
        }
    }
...
void WindowTrackingAdaptor::rotateWindowsInLayout(bool clockwise)
{
    ...
    for (const auto& windowInfo : windowZoneIndices) {
        ...
        QString geometry =
            screenName.isEmpty() ? getZoneGeometry(destZoneId) : getZoneGeometryForScreen(destZoneId, screenName);
        if (geometry.isEmpty()) {
        ...
        qCDebug(lcDbusWindow) << "Window" << windowId << "rotating from zone" << (currentIndex + 1) << "to zone"
                              << (destIndex + 1);
    }
    ...
    for (auto it = m_preFloatZoneAssignments.begin(); it != m_preFloatZoneAssignments.end(); ++it) {
        ...
        if (!oldUuid) {
            qCWarning(lcDbusWindow) << "Invalid zone ID format in pre-float assignment for floating window" << it.key()
                                    << "- zone ID:" << oldZoneId;
        ...
        if (rotationIt != zoneRotationMap.constEnd()) {
            ...
            qCDebug(lcDbusWindow) << "Updating pre-float zone for floating window" << it.key() << "from" << oldZoneId
                                  << "to" << newZoneId;
            it.value() = newZoneId;
        }
    }
...
void WindowTrackingAdaptor::reportNavigationFeedback(bool success, const QString& action, const QString& reason)
{
    ...
    qCDebug(lcDbusWindow) << "KWin effect reported navigation feedback: success=" << success << "action=" << action
                          << "reason=" << reason;
