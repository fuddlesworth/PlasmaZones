                                          ...
                                          bool skipSnapping)
{
    ...
    if (zone.isEmpty()) {
        ...
        Q_EMIT layoutSaveFailed(PI18n::tr("Zone not found", "@info"));
        return;
    }
...
void EditorController::updateZoneName(const QString& zoneId, const QString& name)
{
    if (!m_undoController || !m_zoneManager) {
        ...
        Q_EMIT zoneNameValidationError(zoneId, PI18n::tr("Services not initialized", "@info"));
        return;
    }
    ...
    if (zone.isEmpty()) {
        ...
        Q_EMIT zoneNameValidationError(zoneId, PI18n::tr("Zone not found", "@info"));
        return;
    }
...
void EditorController::updateZoneNumber(const QString& zoneId, int number)
{
    if (!m_undoController || !m_zoneManager) {
        ...
        Q_EMIT zoneNumberValidationError(zoneId, PI18n::tr("Services not initialized", "@info"));
        return;
    }
    ...
    if (zone.isEmpty()) {
        ...
        Q_EMIT zoneNumberValidationError(zoneId, PI18n::tr("Zone not found", "@info"));
        return;
    }
