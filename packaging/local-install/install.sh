...
if [ -d "$SOURCE_DIR/usr" ]; then
    ...
    # Fallback/Dev check: maybe running from source repo
...
if [ -f "$SERVICE_SRC" ]; then
    ...
    # Note: systemd does not expand variables in Environment=, so we set the full path directly
    sed -i "/^\[Service\]/a Environment=\"LD_LIBRARY_PATH=$PREFIX/lib64\"" "$SERVICE_DEST"
else
...
QT_PLUGIN_PATH=$PREFIX/lib64/qt6/plugins:\${QT_PLUGIN_PATH}
QML2_IMPORT_PATH=$PREFIX/lib64/qt6/qml:\${QML2_IMPORT_PATH}
...
echo "This is required for the environment variables to take effect."
echo ""
